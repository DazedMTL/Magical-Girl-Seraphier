...
const ShopScene_ExtensionPluginName =
...
(() => {
  ...
  class Sprite_ActorCharacter extends Sprite_Clickable {
    ...
    updateBitmap(enabled, equipState) {
      ...
      const characterBitmap = ImageManager.loadCharacter(
        this._actor.characterName()
      );
      ...
      if (!this._opacityBitmap || holdPSizeChanged)
  ...
  Window_ShopStatus.prototype.createActorSprites = function () {
    ...
    for (const actor of $gameParty.members()) {
      const sprite = new Sprite_ActorCharacter(
        actor,
        this.clickActor.bind(this)
      );
  ...
  Window_ShopStatus.prototype.clickActor = function (actor) {
    ...
    const index = $gameParty
      .members()
      .map((a) => a.actorId())
      .indexOf(actor.actorId());
    if (index === -1)
      throw new Error(`actorId: ${actor.actorId()} is not found`);
  ...
  Window_ShopStatus.prototype.drawActorEquipInfo = function (x, y, actor) {
    this._currentEquippedItem = this.currentEquippedItem(
      actor,
      this._item.etypeId
    );
    ...
    if (VisibleEquipMode === 1) {
      ...
      // Draw characters
      this.setupActorCharacters(
        x + ActorCharacterBeginOfs,
      ...
      this.drawItemName(
        this._item,
        x + halfWindowWidth + this.rightArrowWidth(),
        y,
        itemNameWidth - halfRightArrowWidth
      );
  ...
  Window_ShopStatus.prototype.setupActorCharacters = function (x, y) {
    ...
    if (this._actorIndex < MaxVisibleActors) {
      actorBegin = 0;
      actorEnd =
        $gameParty.members().length < MaxVisibleActors
    ...
    for (let i = actorBegin; i <= actorEnd; i++) {
      ...
      if (i !== this._actorIndex) {
        actorSprite.changeEquipState(
          false,
          this.getActorEquipState(actorSprite.actor)
        );
      } else {
        actorSprite.changeEquipState(
          true,
  ...
  Window_ShopStatus.prototype.getActorEquipState = function (actor) {
    const currentEquippedItem = this.currentEquippedItem(
      actor,
      this._item.etypeId
    );
    if (!actor.canEquip(this._item)) {
      ...
      this._item &&
      currentEquippedItem &&
      this._item.id === currentEquippedItem.id
    ) {
      return "equipped";
  ...
  Window_ShopStatus.prototype.setupActorCursors = function (y) {
    ...
    this._rightActorArrowSprite.x =
      this.width - this.padding - this._rightActorArrowSprite.width;
  ...
  Window_ShopStatus.prototype.newEquipValue = function (
    actor,
    paramId,
    newItem,
    oldItem
  ...
  Window_ShopStatus.prototype.drawNewParam = function (x, y, paramId) {
    ...
    if (this._actor.canEquip(this._item)) {
      const newValue = this.newEquipValue(
        this._actor,
        paramId,
        this._item,
        this._currentEquippedItem
      );
