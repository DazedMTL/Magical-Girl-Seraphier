...
(() => {
  ...
  function makeCommandListFree(windo) {
    ...
    addData = addData.filter((add) => {
      const dels = delData.filter(
        (del) => del.symbol == add.symbol && del.id == add.id
      );
      return !dels.length;
    });
    // 重複を削除
    ...
    addData.forEach((data) => {
      ...
      switch (data.symbol) {
        ...
        case "skill":
          windo.addCommand(
            data.name || $dataSystem.skillTypes[data.id],
            "skill",
            true,
            data.id
          );
  ...
  function getHelpText(data) {
    ...
    if (data.symbol == "skill") {
      const text = findHelpText(
        [$dataSystem.skillTypes[data.id], "スキル", "skill"],
        data
      );
      if (text) {
        return text;
  ...
  function drawIcon(windo, index) {
    ...
    if (symbol == "guard" && !iconIndex) {
      iconIndex = iconIndex = $dataSkills[guardId]
        ? $dataSkills[guardId].iconIndex
        : 0;
    }
    ...
    if (iconIndex) {
      ...
      iconSprite.x =
        ...
        rect.x +
        (isRight
          ? rect.width + iconTw / 2 - keke_iconPosX
          : -iconTw / 2 + keke_iconPosX);
      iconSprite.y =
        windo.y +
        wPad +
        rect.y +
        iconTw / 2 +
        (rect.height - iconTw * 0.75) / 2 +
        keke_iconPosY;
    }
  }
