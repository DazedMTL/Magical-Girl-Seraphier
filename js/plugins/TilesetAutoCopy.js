  ...
  class TilesetCopyManager {
    ...
    copy(dstId, dstType, srcId, srcType) {
      ...
      if (dstType === "A" && srcType === "A") {
        ...
        throw new Error(
          `dstTypeまたはsrcTypeのどちらかだけがAであってはなりません。`
        );
      } else {
        if (allAType.includes(dstType) || allAType.includes(srcType)) {
          if (dstType !== srcType) {
            throw new Error(
              `A1～A5のコピーの場合、dstTypeとsrcTypeは同じものを指定する必要があります。`
            );
          }
        }
    ...
    _copyFlags(dst, dstIdxBegin, src, srcIdxBegin, srcIdxEnd) {
      for (
        let srcIdx = srcIdxBegin, dstIdx = dstIdxBegin;
        srcIdx <= srcIdxEnd;
        srcIdx++, dstIdx++
      ) {
      ...
      for (const matchData of tileset.note.matchAll(/\<copy\s*\:(.+)\>/g)) {
        if (matchData) {
          ...
          try {
            ...
            throw new Error(
              `メモ欄のフォーマット<copy: ${metaCopy}>の形式は不正です。`
            );
          }
          tilesetCopyManager.copy(
            tileset.id,
            copyInfo.dstType,
            copyInfo.srcId,
            copyInfo.srcType
