...
(() => {
  ...
  if (param.StyleMode === "MVStyle") {
    ...
    param.WindowFrameShow =
      param.WindowFrameShow === undefined ? true : param.WindowFrameShow;
    param.cursorBackShow =
      param.cursorBackShow === undefined ? false : param.cursorBackShow;
    param.ActorFaceVisible =
      param.ActorFaceVisible === undefined ? false : param.ActorFaceVisible;
    ...
    param.WindowShow = param.WindowShow === undefined ? true : param.WindowShow;
    param.WindowFrameShow =
      param.WindowFrameShow === undefined ? false : param.WindowFrameShow;
    param.cursorBackShow =
      param.cursorBackShow === undefined ? true : param.cursorBackShow;
    param.ActorFaceVisible =
      param.ActorFaceVisible === undefined ? true : param.ActorFaceVisible;
  ...
  Game_Actor.prototype.isSpriteVisible = function () {
    return !$gameSystem.isSideView() && param.ActorEffectShow
      ? param.ActorEffectShow
      : _Game_Actor_isSpriteVisible.call(this);
  };
  ...
  const _BattleManager_displayEscapeSuccessMessage =
  ...
  const _BattleManager_displayEscapeFailureMessage =
  ...
  const _Scene_Battle_createActorWindow =
  ...
  Scene_Battle.prototype.differenceX = function () {
    if (param.ActorStatusWindowOnPosition) {
      if (param.ActorStatusWindowCenter) {
        return (
          (Graphics.boxWidth - this._statusWindow.width) / 2 +
          param.ActorStatusWindow_X
        );
      } else {
        return (
          (Graphics.boxWidth - Graphics.width) / 2 + param.ActorStatusWindow_X
        );
      }
    }
  ...
  Scene_Battle.prototype.differenceY = function () {
    if (param.ActorStatusWindowOnPosition) {
      return (
        (Graphics.boxHeight - Graphics.height) / 2 +
  ...
  const _Scene_Battle_statusWindowRect =
  ...
  Scene_Battle.prototype.statusWindowRect = function () {
    ...
    if (param.StyleMode === "Default" || param.StyleMode === "MVStyle") {
      ...
      rect.y +=
        (Graphics.height - Graphics.boxHeight) / 2 +
        (param.WindowFrameShow ? 4 : 0);
      rect.height -= param.WindowFrameShow ? extra : 0;
    } else if (param.StyleMode === "XPStyle") {
      rect.width =
        param.ActorStatusWindow_Width > 0
          ? param.ActorStatusWindow_Width - 8
          : Graphics.boxWidth;
      rect.height =
        param.ActorStatusWindow_Height > 0
          ? param.ActorStatusWindow_Height
          : this.windowAreaHeight() + extra - (param.WindowFrameShow ? 10 : 0);
      if (param.ActorStatusWindowOnPosition) {
        rect.x =
          (param.ActorStatusWindowCenter
            ? (Graphics.width - rect.width) / 2
            : 0) + param.ActorStatusWindow_X;
  ...
  Scene_Battle.prototype.partyCommandWindowRect = function () {
    ...
    if (param.StyleMode === "Default" || param.StyleMode === "MVStyle") {
      ...
      rect.width =
        param.PartyCommand_Width > 0
          ? param.PartyCommand_Width
          : Graphics.boxWidth;
      rect.height = this.partyWindowAreaHeight();
      rect.x = param.PartyCommandCenter
        ? Graphics.boxWidth / 2 - rect.width / 2
        : 0 + param.PartyCommand_X;
    ...
    if (param.ActorCommandMode === 0) {
      ...
      return (
        (param.ActorCommandCenter
          ? Graphics.boxWidth / 2 - this.actorCommandWidth() / 2
          : 0) + param.ActorCommand_X
      );
    } else {
      return 0;
    }
  ...
  const _Scene_Battle_updateStatusWindowPosition =
  ...
  Scene_Battle.prototype.statusWindowX = function () {
    if (this.isAnyInputWindowActive()) {
      ...
      return (
        this.statusWindowRect().x +
        (this._partyCommandWindow.width / 2) *
          (this.isRightInputMode() ? 1 : -1)
      );
    }
  };
  ...
  Scene_Battle.prototype.partyCommand_YPosition = function () {
    if (param.PartyCommandPosition === 0) {
      ...
      return (
        Graphics.boxHeight -
        this.windowAreaHeight() -
        this.partyWindowAreaHeight()
      );
    }
  };
  ...
  const _Scene_Battle_selectPreviousCommand =
  ...
  const _Scene_Battle_endCommandSelection =
  ...
  Scene_Battle.prototype.actorWindowOpacity = function () {
    this._statusWindow.opacity = param.WindowShow
      ? param.ActorWindowSelectOpacity || 255
      : 0;
  ...
  Scene_Battle.prototype.activeWindow = function () {
    //SkillWindowOpacity
    return (
      this.opacityskillWindow() ||
      this.opacityItemWindow() ||
      this.opacityEnemyWindow() ||
      this.opacityMessageWindow() ||
      BattleManager.actorStatusWindowOpacity
    );
  };
  ...
  Scene_Battle.prototype.opacityMessageWindow = function () {
    return (
      this.statusWindow_Top <= this._messageWindow.y &&
      this.statusWindow_Under >= this._messageWindow.y &&
      this._messageWindow.onMessage
    );
  };
  ...
  Window_PartyCommand.prototype.maxCols = function () {
    return param.StyleMode === "Default" || param.StyleMode === "MVStyle"
      ? 1
      : Math.ceil(
          this._list
            ? Math.min(this._list.length, param.PartyCommandMaxCol)
  ...
  Window_PartyCommand.prototype.refresh = function () {
    ...
    if (param.StyleMode === "Default" || param.StyleMode === "MVStyle") {
      ...
      this.height = this.fittingHeight(
        Math.min(
          Math.ceil(this.maxItems() / param.PartyCommandMaxCol),
      ...
      if (param.PartyCommandPosition === 2) {
        this.y =
          Graphics.boxHeight -
          (this.height + this.statusWindowHeight) +
          (param.WindowFrameShow ? 0 : 6) +
  ...
  Window_ActorCommand.prototype.maxCols = function () {
    return param.StyleMode === "Default" || param.StyleMode === "MVStyle"
      ...
      : Math.ceil(
          this._list
            ? Math.min(this._list.length, param.ActorCommandMaxCol)
  ...
  Window_ActorCommand.prototype.refresh = function () {
    ...
    if (param.StyleMode === "Default" || param.StyleMode === "MVStyle") {
      ...
      if ((actorIndex >= 0 || this._actor) && param.ActorCommandMode === 0) {
        ...
        this.height = this.fittingHeight(
          Math.min(
            Math.ceil(this.maxItems() / param.ActorCommandMaxCol),
            param.ActorCommandMaxRow
          )
        );
        this.width =
          param.ActorCommand_Width > 0
            ? param.ActorCommand_Width
            : Math.min(this.width, rect.width);
        this.x = Math.max(
          -this._statusWindow.UI_Difference + 4,
          (rect.width - this.width) / 2 +
            rect.x +
            this.itemPadding() +
            Math.max(param.ActorCommand_X, 0) +
            this._statusWindow.differenceX
        );
        this.x = Math.min(
          this.x,
          Graphics.boxWidth + this._statusWindow.UI_Difference - this.width - 4
        );
        let zeroPosition = 0;
        if (param.ActorStatusWindowOnPosition) {
          this.y =
            this._statusWindow.differenceY -
            this.height +
          ...
          this.y =
            Graphics.boxHeight -
            (this.height + this._statusWindow.height) +
            param.ActorCommand_Y +
            rect.y;
        }
        ...
        (actorIndex >= 0 || this._actor) &&
        param.ActorCommandMode >= 1
      ) {
        this.height = this.fittingHeight(
          Math.min(
            Math.ceil(this.maxItems() / param.ActorCommandMaxCol),
  ...
  Window_BattleStatus.prototype.initialize = function (rect) {
    ...
    if (BattleManager.isBattleTest()) {
      console.log(
        "アクターステータス横幅 " +
          (this.itemWidth() - this.colSpacing()) +
          " アクターステータス縦幅 " +
          (this.itemHeight() - this.rowSpacing())
      );
    }
  };

  Window_BattleStatus.prototype.maxCols = function () {
    return param.StyleMode === "MVStyle"
      ? 1
      : param.ActorMaxCol > 0
      ? param.ActorMaxCol
      : Math.max($gameParty.maxBattleMembers(), 4);
  };

  const _Window_BattleStatus_itemHeight =
    Window_BattleStatus.prototype.itemHeight;
  Window_BattleStatus.prototype.itemHeight = function () {
    const row =
      param.ActorMaxRow > 0
        ? param.ActorMaxRow
        : Math.ceil($gameParty.maxBattleMembers() / this.maxCols());
    return Math.floor(_Window_BattleStatus_itemHeight.call(this) / row);
  };

  const _Window_BattleStatus_rowSpacing =
    Window_BattleStatus.prototype.rowSpacing;
  Window_BattleStatus.prototype.rowSpacing = function () {
    return Math.ceil($gameParty.maxBattleMembers() / this.maxCols()) > 1
      ? 4
      : _Window_BattleStatus_rowSpacing.call(this);
  };
  ...
  Window_BattleStatus.prototype.drawItemBackground = function (index) {
    ...
    if (this._actorBack[index]) {
      ...
      (param.WindowShow && param.cursorBackShow) ||
      param.cursorBackShow
    ) {
  ...
  Window_BattleStatus.prototype.preparePartyRefresh = function () {
    ...
    this._actorBack = [];
    const bitmap = param.actorBackground
      ? ImageManager.loadSystem(param.actorBackground)
      : null;
  ...
  const _Window_BattleStatus_refreshCursor =
    Window_BattleStatus.prototype.refreshCursor;
  Window_BattleStatus.prototype.refreshCursor = function () {
    if (
      (param.SelectBackShow && this.constructor === Window_BattleStatus) ||
      (param.ActorSelectBackShow && this.constructor === Window_BattleActor)
    ) {
  ...
  Window_BattleStatus.prototype.statusPosition = function (index, rect) {
    ...
    if (param.StyleMode === "MVStyle") {
      ...
      if (param.ActorStatusMode === 1) {
        rect.x +=
          Math.floor(this.width / 2 - (itemWidth * maxCols) / 2) -
          this.itemPadding();
  ...
  const _Window_BattleStatus_placeTimeGauge =
    Window_BattleStatus.prototype.placeTimeGauge;
  ...
  Window_BattleStatus.prototype.placeBasicGauges = function (actor, x, y) {
    if (param.StyleMode === "MVStyle") {
      ...
      x2 = param.MPChangePosition
        ? param.ActorMP_X + this._rect.x
        : x + ($dataSystem.optDisplayTp ? 123 : 216);
      ...
      y2 = param.MPChangePosition
        ? param.ActorMP_Y + this._rect.y
        : y + this.gaugeLineHeight();
      this.placeGauge(actor, "mp", x2, y2);
      if ($dataSystem.optDisplayTp) {
        x2 = param.TPChangePosition ? param.ActorTP_X + this._rect.x : x;
        y2 = param.TPChangePosition
          ? param.ActorTP_Y + this._rect.y
          : y + this.gaugeLineHeight() * 2;
        this.placeGauge(actor, "tp", x2, y2);
      }
    }
  };

  const _Window_BattleStatus_placeStateIcon =
    Window_BattleStatus.prototype.placeStateIcon;
  Window_BattleStatus.prototype.placeStateIcon = function (actor, x, y) {
    _Window_BattleStatus_placeStateIcon.call(this, actor, x, y);
  };

  const _Window_BattleStatus_placeActorName =
    Window_BattleStatus.prototype.placeActorName;
  ...
  Window_BattleStatus.prototype.faceRect = function (index) {
    ...
    rect.height =
      param.ActorFace_Height > 0 ? param.ActorFace_Height : rect.height;
    return rect;
  };
  ...
  Window_BattleStatus.prototype.setActorWindow = function (
  ...
  Window_BattleStatus.prototype.stateIconX = function (rect) {
    if (param.StyleMode === "MVStyle") {
      return param.StateChangePosition
        ? param.ActorState_X + rect.x
        : rect.x + this.basicGaugesX(rect) - 32;
    }
    return param.StateChangePosition
      ? param.ActorState_X + rect.x
      : _Window_BattleStatus_stateIconX.call(this, rect);
  };

  const _Window_BattleStatus_stateIconY =
    Window_BattleStatus.prototype.stateIconY;
  Window_BattleStatus.prototype.stateIconY = function (rect) {
    if (param.StyleMode === "MVStyle") {
      return param.StateChangePosition
        ? param.ActorState_Y + rect.y
        : rect.y + rect.height / 2;
    }
    return param.StateChangePosition
      ? param.ActorState_Y + rect.y
      : _Window_BattleStatus_stateIconY.call(this, rect);
  };
  ...
  const _Window_BattleStatus_basicGaugesX =
    Window_BattleStatus.prototype.basicGaugesX;
  Window_BattleStatus.prototype.basicGaugesX = function (rect) {
    return (
      _Window_BattleStatus_basicGaugesX.call(this, rect) +
      (param.StyleMode === "MVStyle" ? rect.width - 338 : 0)
    );
  };
  ...
  Window_BattleActorImges.prototype = Object.create(
  ...
  Window_BattleActorImges.prototype.loadBitmap = function (deta) {
    deta.stateBitmap = [];
    deta.defaultBitmap = Imported.NUUN_ActorPicture
      ? ImageManager.nuun_actorPictures(deta.defaultImg)
      : loadBattleStyleActorImg(deta.defaultImg);
    if (deta.deathImg) {
      deta.deathBitmap = Imported.NUUN_ActorPicture
        ? ImageManager.nuun_actorPictures(deta.deathImg)
        : loadBattleStyleActorImg(deta.deathImg);
    }
    if (deta.dyingImg) {
      deta.dyingBitmap = Imported.NUUN_ActorPicture
        ? ImageManager.nuun_actorPictures(deta.dyingImg)
        : loadBattleStyleActorImg(deta.dyingImg);
    }
    if (deta.damageImg) {
      deta.damageBitmap = Imported.NUUN_ActorPicture
        ? ImageManager.nuun_actorPictures(deta.damageImg)
        : loadBattleStyleActorImg(deta.damageImg);
    }
    if (deta.victoryImg) {
      deta.victoryBitmap = Imported.NUUN_ActorPicture
        ? ImageManager.nuun_actorPictures(deta.victoryImg)
        : loadBattleStyleActorImg(deta.victoryImg);
    }
    if (deta.chantImg) {
      deta.chantBitmap = Imported.NUUN_ActorPicture
        ? ImageManager.nuun_actorPictures(deta.chantImg)
        : loadBattleStyleActorImg(deta.chantImg);
    }
    if (deta.stateImg) {
      for (const listdeta of deta.stateImg) {
        if (listdeta.actorStateImg && listdeta.stateImgId > 0) {
          deta.stateBitmap[listdeta.stateImgId] = {};
          deta.stateBitmap[listdeta.stateImgId].imges =
            Imported.NUUN_ActorPicture
              ? ImageManager.nuun_actorPictures(listdeta.actorStateImg)
              : loadBattleStyleActorImg(listdeta.actorStateImg);
          deta.stateBitmap[listdeta.stateImgId].always = listdeta.Always
            ? true
            : false;
          deta.stateBitmap[listdeta.stateImgId].priorityId =
            listdeta.priorityId;
        }
      }
    }
    ...
    if (deta.stateFaceIndex) {
      for (const listdeta of deta.stateFaceIndex) {
        if (listdeta.actorStateIndex >= 0 && listdeta.stateFaceId > 0) {
          ...
          deta.stateBitmap[listdeta.stateFaceId].FaceId =
            listdeta.actorStateIndex;
          deta.stateBitmap[listdeta.stateFaceId].always = listdeta.Always
            ? true
            : false;
          deta.stateBitmap[listdeta.stateFaceId].priorityId =
            listdeta.priorityId;
        }
      }
    }
  ...
  Window_BattleActorImges.prototype.drawItemButler = function (
    index,
    actor,
    deta
  ) {
    ...
    const x =
      ...
      Math.floor(
        (this.itemWidth() -
          (deta.defaultBitmap.width * deta.Actor_Scale) / 100) /
          2
      ) +
      4 +
      param.ActorImg_X +
    ...
    const y =
      rect.y +
      rect.height -
      Math.floor((deta.defaultBitmap.height * deta.Actor_Scale) / 100) +
      7 +
      param.ActorImg_Y +
      deta.Actor_Y;
  ...
  Window_BattleActorImges.prototype.drawItemFace = function (
    index,
    actor,
    deta
  ) {
    ...
    const sy = Math.floor(
      Math.floor(actor.faceIndex() / 4) * ph + (ph - sh) / 2
    );
  ...
  Window_BattleActorImges.prototype.createActorImgSprite = function (
    key,
    spriteClass
  ) {
  ...
  Window_BattleActorStatus.prototype = Object.create(
    Window_BattleStatus.prototype
  );
  ...
  Window_BattleActorStatus.prototype.refreshContentsDraw = function () {
    ...
    for (const actor of $gameParty.members()) {
      ...
      const rect = this.itemRectWithPadding(index);
      const stateIconX = param.StateChangePosition
        ? param.ActorState_X + rect.x
        : this.stateIconX(rect);
      const stateIconY = param.StateChangePosition
        ? param.ActorState_Y + rect.y
        : this.stateIconY(rect);
      this.drawStateIcon(actor, stateIconX, stateIconY);
    }
  };

  //Window_BattleActor
  const _Window_BattleActor_initialize =
  ...
  const _Window_BattleLog_displayRemovedStates =
  ...
  const _Window_BattleLog_displayBuffs =
    Window_BattleLog.prototype.displayBuffs;
  Window_BattleLog.prototype.displayBuffs = function (target, buffs, fmt) {
    ...
    if (result.addedBuffs.length > 0 || result.addedDebuffs.length > 0) {
      for (const paramId of buffs) {
        const id = target.result().isDebuffAdded(paramId)
          ? paramId + 10
          : paramId;
  ...
  const _Window_BattleLog_displayChangedBuffs =
  ...
  const _Sprite_Battler_updateVisibility =
    Sprite_Battler.prototype.updateVisibility;
  ...
  Sprite_Actor.prototype.actorHomeRefresh = function (index) {
    ...
    if (param.ActorStatusMode === 0) {
      x =
        ...
        this.differenceX() +
        this._statusWindow.itemPadding();
    } else if (param.ActorStatusMode === 2) {
      x =
        width -
        Math.min(this._statusWindow.maxItems(), maxCols) * itemWidth +
        w_index * itemWidth +
        Math.floor(itemWidth / 2) +
        this.differenceX() -
        this._statusWindow.itemPadding();
      ...
      x =
        width / 2 -
        (Math.min(this._statusWindow.maxItems(), maxCols) * itemWidth) / 2 +
        w_index * itemWidth +
        Math.floor(itemWidth / 2) +
        this.differenceX();
    }
  ...
  Sprite_Actor.prototype.differenceX = function () {
    if (param.ActorStatusWindowOnPosition) {
      if (param.ActorStatusWindowCenter) {
        ...
        return (
          0 +
          (Graphics.boxWidth - Graphics.width) / 2 +
          param.ActorStatusWindow_X
        );
      }
    }
  ...
  Sprite_Actor.prototype.differenceY = function () {
    if (param.ActorStatusWindowOnPosition) {
      return (
        (Graphics.boxHeight - Graphics.height) / 2 + param.ActorStatusWindow_Y
      );
    }
  ...
  Sprite_Actor.prototype.setBattler = function (battler) {
    ...
    if (
      ...
      battler === this._actor &&
      !$gameSystem.isSideView() &&
      $gameTemp.isBattleEffectsRefresh() &&
      param.ActorEffectShow
    ) {
  ...
  Sprite_Actor.prototype.damageOffsetX = function () {
    return (
      ($gameSystem.isSideView()
        ? _Sprite_Actor_damageOffsetX.call(this)
        : Sprite_Battler.prototype.damageOffsetX.call(this)) +
      param.ActorDamage_X
    );
  };
  ...
  Sprite_Actor.prototype.damageOffsetY = function () {
    return (
      ($gameSystem.isSideView() ? 0 : 0) +
      _Sprite_Actor_damageOffsetY.call(this) +
      param.ActorDamage_Y
    );
  };
  ...
  Sprite_ActorImges.prototype.setDead = function () {
    const mode = this.faceMode();
    if (
      ((mode && this._deta.deathFaceIndex < 0) ||
        (!mode && !this._deta.deathBitmap)) &&
      param.imgDeathHide
    ) {
  ...
  Sprite_ActorImges.prototype.setRevive = function () {
    ...
    if (
      this._battler.isDying() &&
      ((mode && this._deta.dyingFaceIndex > 0) ||
        (!mode && this._deta.dyingBitmap))
    ) {
      this._imgIndex = 2;
    } else {
      this._imgIndex = 0;
  ...
  Sprite_ActorImges.prototype.setVictory = function () {
    const mode = this.faceMode();
    if (
      (mode && this._deta.victoryFaceIndex > 0) ||
      (!mode && this._deta.victoryBitmap)
    ) {
  ...
  Sprite_ActorImges.prototype.setChant = function () {
    const mode = this.faceMode();
    if (
      (mode && this._deta.chantFaceIndex > 0) ||
      (!mode && this._deta.chantBitmap && this._updateCount <= 0)
    ) {
      this._updateCount = 1;
      this._imgIndex = 4;
    }
  ...
  Sprite_ActorImges.prototype.setDying = function () {
    ...
    if (
      (mode && this._deta.dyingFaceIndex > 0) ||
      (!mode && this._deta.dyingBitmap && this._updateCount <= 0)
    ) {
      this._updateCount = 1;
      this._imgIndex = 2;
    }
  ...
  Sprite_ActorImges.prototype.setDamage = function () {
    const mode = this.faceMode();
    if (
      (mode && this._deta.damageFaceIndex > 0) ||
      (!mode && this._deta.damageBitmap)
    ) {
      this._imgIndex = 3;
  ...
  Sprite_ActorImges.prototype.stateImg = function (mode) {
    ...
    actor._states.forEach(function (stateId) {
      ...
      const changeImgId = state.meta.ChangeImgId
        ? Number(state.meta.ChangeImgId)
        : 0;
      if (faceMode) {
        if (
          this._deta.stateBitmap[changeImgId] &&
          this._deta.stateBitmap[changeImgId].FaceId >= 0 &&
          mode === this._deta.stateBitmap[changeImgId].always
        ) {
        ...
        if (
          this._deta.stateBitmap[changeImgId] &&
          this._deta.stateBitmap[changeImgId].imges &&
          mode === this._deta.stateBitmap[changeImgId].always
        ) {
          priorityId = this._deta.stateBitmap[changeImgId].priorityId;
          if (priority < priorityId) {
  ...
  Sprite_BattleGauge.prototype.initialize = function () {
    ...
    this._HPGaugeWidth =
      param.StyleMode === "MVStyle"
        ? param.HPGaugeWidth || ($dataSystem.optDisplayTp ? 108 : 201)
        : param.HPGaugeWidth;
    this._MPGaugeWidth =
      param.StyleMode === "MVStyle"
        ? param.MPGaugeWidth || ($dataSystem.optDisplayTp ? 96 : 114)
        : param.MPGaugeWidth;
    this._GaugeHeight = 0;
  };
  ...
  //Spriteset_Base
  const _Spriteset_Base_makeTargetSprites =
    Spriteset_Base.prototype.makeTargetSprites;
  Spriteset_Base.prototype.makeTargetSprites = function (targets) {
    ...
    if (this.constructor === Spriteset_Battle) {
      this._effectsContainer = this.animationTarget(targetSprites)
        ? this._effectsFrontContainer
        : this._effectsBackContainer;
    }
  ...
  Spriteset_Base.prototype.animationTarget = function (targetSprites) {
    if (!$gameSystem.isSideView() && param.ActorEffectShow) {
      const target = targetSprites.find(
        (targets) => targets.constructor === Sprite_Actor
      );
  ...
  const _Spriteset_Battle_loadSystemImages =
    Spriteset_Battle.prototype.loadSystemImages;
  ...
  Spriteset_Battle.prototype.updateBackground = function () {
    ...
    this._backgroundSprite.y =
      Graphics.height - this._backgroundSprite.bitmap.height;
    this._backgroundVisible = true;
  };

  const _Spriteset_Battle_createBattleField =
  ...
  Sprite_BattleStylePopUp.prototype.popUpState = function (id, mode) {
    ...
    const popUpName = state.meta.PopUpStateName
  ...
  Sprite_BattleStylePopUp.prototype.popUpBuff = function (id, mode) {
    const find = param.PopUpBuff.find((buff) => buff.StateType === id);
    const popUpName = find.PopUpStateName
      ? find.PopUpStateName
      : id < 10
      ? TextManager.param(id) + "上昇"
      : TextManager.param(id - 10) + "低下";
