...
(function () {
  ...
  let pActorWindowPositionH = setDefault(
    parameters["ActorWindowPositionH"],
    "right"
  );
  const pActorWindowPositionV = setDefault(
    parameters["ActorWindowPositionV"],
    "middle"
  );
  const pDisplayBenchMembers = toBoolean(
    parameters["DisplayBenchMembers"],
    true
  );
  ...
  const pEquipSymbolEquipColor = toNumber(
    parameters["EquipSymbolEquipColor"],
    0
  );
  ...
  const pEquipSymbolEqualColor = toNumber(
    parameters["EquipSymbolEqualColor"],
    0
  );
  ...
  if (pAboveHelpWindow) {
     ...
     * ●コマンドウィンドウの作成
     */
    const _Scene_Shop_createCommandWindow =
  ...
  if (pDisplayActorWindow) {
     ...
     * ●購入ウィンドウの有効化
     */
    const _Scene_Shop_activateBuyWindow =
      Scene_Shop.prototype.activateBuyWindow;
     ...
     * ●売却ウィンドウの有効化
     */
    const _Scene_Shop_activateSellWindow =
  ...
  Window_ShopActor.prototype.currentEquippedItem = function (actor, etypeId) {
    return Window_ShopStatus.prototype.currentEquippedItem.apply(
      this,
      arguments
    );
  };
   ...
   * ●装備アイテムかどうかの判定
   */
  const _Window_ShopStatus_isEquipItem =
    Window_ShopStatus.prototype.isEquipItem;
   ...
   * 【上書】装備情報の描画
   */
  const _Window_ShopStatus_drawEquipInfo =
