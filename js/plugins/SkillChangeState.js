...
(() => {
  ...
  const stateAndSkills = JSON.parse(parameters.stateAndSkills || "[]").map(
    (e) => {
      const stateAndSkill = JSON.parse(e);
      return {
        state: Number(stateAndSkill.state || 0),
        skills: JSON.parse(stateAndSkill.skills || "[]").map((skillId) =>
          Number(skillId || 0)
        ),
      };
    }
  ...
  function Game_Actor_SkillChangeStateMixIn(gameActor) {
    ...
    gameActor.skills = function () {
      const s = stateAndSkills.find((stateAndSkill) =>
        this.isStateAffected(stateAndSkill.state)
      );
      if (s) {
