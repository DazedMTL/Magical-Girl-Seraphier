...
(() => {
  ...
  Game_BattlerBase.prototype.meetsSkillConditions = function (skill) {
    return (
      this.itemConditionsEX(skill) &&
      _Game_BattlerBase_meetsSkillConditions.call(this, skill)
    );
  };

  const _Game_BattlerBase_meetsItemConditions =
    Game_BattlerBase.prototype.meetsItemConditions;
  Game_BattlerBase.prototype.meetsItemConditions = function (item) {
    return (
      this.itemConditionsEX(item) &&
      _Game_BattlerBase_meetsItemConditions.call(this, item)
    );
  };
  ...
  Game_Actor.prototype.itemConditionsEX = function (item) {
    ...
    if (targets.length > 0) {
      ...
      while (true) {
        ...
        const mode =
          (id > 1
            ? Number(item.meta["MatchMode" + id])
            : Number(item.meta.MatchMode)) || 0;
        for (const target of targets) {
          if (
            this.getTriggerConditions(
              item,
              target,
              "ConditionalSkill" + tagId,
              "TargetConditionalSkill" + tagId,
              "PartyConditionalSkill" + tagId,
              "TroopConditionalSkill" + tagId,
              null,
              null,
              mode
            )
  ...
  function checkTag(item, id) {
    ...
    if (id === 1) {
      return (
        meta.ConditionalSkill ||
        meta.TargetConditionalSkill ||
        meta.PartyConditionalSkill ||
        meta.TroopConditionalSkill
      );
    } else {
      return (
        meta["ConditionalSkill" + id] ||
        meta["TargetConditionalSkill" + id] ||
        meta["PartyConditionalSkill]" + id] ||
        meta["TroopConditionalSkill" + id]
      );
    }
  }
})();
