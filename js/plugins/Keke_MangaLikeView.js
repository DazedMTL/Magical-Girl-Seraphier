...
(() => {
  ...
  PluginManager.registerCommand(pluginName, "serifSet", (args) => {
    ...
    for (const type of typeList) {
      ...
      set_[type + "Serifs"].forEach((serif) => {
        if (serif["スキルID"]) {
          serif["ID"] = serif["スキルID"];
        }
        if (serif["アイテムID"]) {
          serif["ID"] = serif["アイテムID"];
        }
      }, this);
  ...
  PluginManager.registerCommand(pluginName, "instantSerif", (args) => {
    ...
    if (!$gameSystem._instantSerifsKe) {
      $gameSystem._instantSerifsKe = {};
    }
  ...
  Game_System.prototype.setEachSerifKe = function (set_, type) {
    if (!set_[type + "Serifs"].length) {
      return;
    }
    if (!this[type + "SerifsKe"]) {
      this[type + "SerifsKe"] = {};
    }
    const key = set_.type + set_.id;
  ...
  function callOpenMangaView() {
    ...
    const party = $gameParty
      .aliveMembers()
      .filter((actor) => $gameSystem.getEachSerifKe(actor, "open"));
    ...
    const troop = $gameTroop
      .aliveMembers()
      .filter((enemy) => $gameSystem.getEachSerifKe(enemy, "open"));
  ...
  BattleManager.startAction = function () {
    ...
    // マンガビューの表示(アクト)
    setTimeout(showMangaView, 10, this._subject, "act", {
      action: this._action,
    });
    // ファイナルアクターを保存
    if (this._subject._actorId) {
      this._finalActorKeMglv = this._subject;
    }
    // ファイナルエネミーを保存
    if (this._subject._enemyId) {
      this._finalEnemyKeMglv = this._subject;
    }
  };
  ...
  BattleManager.processVictory = function () {
    ...
    if (!subje || !$gameSystem.getEachSerifKe(subje, "victory")) {
      const party = $gameParty
        .aliveMembers()
        .filter((actor) => $gameSystem.getEachSerifKe(actor, "victory"));
  ...
  BattleManager.processDefeat = function () {
    ...
    if (!subje || !$gameSystem.getEachSerifKe(subje, "victory")) {
      const troop = $gameTroop
        .aliveMembers()
        .filter((enemy) => $gameSystem.getEachSerifKe(enemy, "victory"));
    ...
    chainSerifs.forEach((serif, i) => {
      ...
      actorIds.forEach((id) =>
        $gameParty
          .members()
          .forEach((b) =>
            b._actorId == id && b.isAppeared() && b.isAlive()
              ? actors.push(b)
              : 0
          )
      );
      ...
      enemyIds.forEach((id) =>
        $gameTroop
          .members()
          .forEach((b) =>
            b._enemyd == id && b.isAppeared() && b.isAlive()
              ? enemies.push(b)
              : 0
          )
      );
      ...
      targets.forEach((battler, i) => {
        ...
        // 出さない条件
        if (battler._mangaViewCountKe && battler._mangaViewTypeKe == "open") {
          return;
        }
        // セット
        battler._mangaViewPacksKe.push(pack);
  ...
  Sprite_Battler.prototype.createMangaView3Ke = function () {
    ...
    const showDire =
      pack.type == "victory"
        ? keke_showDireVictory
        : battler._actorId
        ? keke_showDireActor
        : keke_showDireEnemy;
    if (pack.serifUdReverse) {
      showDire = showDire == "上" ? "下" : "上";
    }
    const asi = getFullAnimeStatusAsi(battler);
    let data = null;
    let match = null;
    ...
    if (textStyle["小窓表示"] != "表示しない") {
      if (textStyle["小窓表示"] == "キャラ名") {
        ...
        textStyle["小窓表示"] == "技名" &&
        (type == "skill" || type == "item" || type == "chain")
      ) {
    ...
    if (keke_showAnime["時間"]) {
      ...
      this._mangaViewsKe.forEach((sprite) => {
        ...
        if (keke_showAnime["方向"].includes("横")) {
          sprite.scale.x = 0;
        }
        if (keke_showAnime["方向"].includes("縦")) {
          sprite.scale.y = 0;
        }
      }, this);
    }
  ...
  function getStyleByNote(battler, type, id, action, item, typeName) {
    ...
    let baseStyle = null;
    let note = item
      ? item.meta["viewStyle"] || item.meta["ビュースタイル"]
      : null;
    if (note) {
  ...
  function openSerifSet(battler, type, id, action, item, serifSet) {
    ...
    if (serifSet) {
      ...
      // スタイル
      for (const style of keke_commonTextStyle) {
        if (style["名"] == serifSet["テキストスタイル"]) {
          textStyle = style;
          break;
        }
      }
      for (const style of keke_commonBaseStyle) {
        if (style["名"] == serifSet["ベーススタイル"]) {
          baseStyle = style;
          break;
        }
      }
  ...
  function getStyleBasic(textStyle, baseStyle, typeName) {
    if (!textStyle) {
      for (const style of keke_commonTextStyle) {
        if (style["名"] == keke_styleBasic[`${typeName}テキストスタイル`]) {
          textStyle = style;
          break;
        }
      }
    ...
    if (!baseStyle) {
      for (const style of keke_commonBaseStyle) {
        if (style["名"] == keke_styleBasic[`${typeName}ベーススタイル`]) {
          baseStyle = style;
          break;
        }
      }
  ...
  Sprite_Battler.prototype.deleteMangaView2Ke = function () {
    ...
    const spriteset = scene._spriteset;
    const layer =
      asi && asi.faceBaseSprite
        ? cnst._layers[cnst._messageLayer]
  ...
  Sprite_Battler.prototype.moveMangaViewKe = function () {
    ...
    if (!this._mangaViewsKe || !this._mangaViewsKe.length) {
      return;
    }
    // イニット
    const battler = this._battler;
  ...
  // マンガビューの移動(フルアニメ)
  Sprite_Battler.prototype.moveMangaViewFullAnimeKe = function (
    asi,
    gDownMax,
    gLeftMax,
    gRightMax,
    ...
    // 初期化
    if (this._mangaViewPreXKe == null) {
      this._mangaViewPreXKe = faceBaseSprite.x;
    }
    if (this._mangaViewPreYKe == null) {
    ...
    if (moveX && (!isAct || dire == "x")) {
      views.forEach(
        (sprite) => (sprite.x = faceBaseSprite.x + sprite._relativeX)
      );
      // 画面外に出さない
      const vLeft =
        faceBaseSprite.x + baseSprite._relativeX + baseSprite._edgeLeft;
      if (vLeft < gLeftMax) {
        views.forEach((sprite) => (sprite.x += gLeftMax - vLeft));
      }
      const vRight =
        faceBaseSprite.x + baseSprite._relativeX + baseSprite._edgeRight;
      if (vRight > gRightMax) {
    ...
    if (moveY && (!isAct || dire == "y")) {
      views.forEach(
        (sprite) => (sprite.y = faceBaseSprite.y + sprite._relativeY)
      );
      // 画面外に出さない
      ...
      const vDown =
        faceBaseSprite.y + baseSprite._relativeY + baseSprite._edgeDown;
      if (vUp > gDownMax) {
  ...
  // マンガビューの移動(通常)
  Sprite_Battler.prototype.moveMangaViewNormalKe = function (
    gDownMax,
    gLeftMax,
    gRightMax,
    gUpMax
  ...
  // フロントビュー時のアニメ条件
  const _Spriteset_Battle_makeTargetSprites =
    Spriteset_Battle.prototype.makeTargetSprites;
  ...
  Sprite_Actor.prototype.updatePosFrontViewKeMglv = function () {
    if (!this.validFrontViewAdaptKeMglv()) {
      return;
    }
    const statusWindow = SceneManager._scene._statusWindow;
  ...
  function createTextBox(
    ...
    // ベーススプライトの形成
    const baseData = createBaseSprite(
      x,
      y,
      textW,
      textH,
    ...
    // テキストポックスを画面外に出さない
    const noOuterData = noOuterTextBox(
      allSprites,
      cfgs,
      textW,
      textLines,
  ...
  function calcIconSprite(icons, textStyle, textLines) {
    ...
    iconSprite.x =
      textLines.x[m] +
      (icons.faceH ? textStyle["顔グラX"] || 0 : textStyle["アイコンX"] || 0);
    iconSprite.y =
      textLines.y[m] +
      (icons.faceH ? textStyle["顔グラY"] || 0 : textStyle["アイコンY"] || 0);
    // 位置補正
  ...
  function createMiniTextSprite(minis, textSprite, textW, textLines, iconW) {
    ...
    // ミニベース形成
    const miniBaseData = createBaseSprite(
      0,
      0,
      miniTextMaxW,
      miniTextTotalH,
  ...
  function noOuterTextBox(
    ...
    const maxDown = Math.max(
      baseH / 2,
      minis.text && miniMap.includes("下")
        ? miniTextSprite.y - baseSprite.y + miniBaseH / 2
        : 0
    );
    const maxLeft = Math.max(
      baseW / 2,
      topW / 2 + (iconSprite ? iconW : 0),
      minis.text && miniMap.includes("左")
        ? baseSprite.x - miniTextSprite.x + miniBaseW / 2
        : 0
    );
    const maxRight = Math.max(
      baseW / 2,
      minis.text && miniMap.includes("右")
        ? miniTextSprite.x - baseSprite.x + miniBaseW / 2
        : 0
    );
    const maxUp = Math.max(
      baseH / 2,
      minis.text && miniMap.includes("上")
        ? baseSprite.y - miniTextSprite.y + miniBaseH / 2
        : 0
    );
    ...
    const gDownMax =
      cfgs.forceDownMax != null
        ? gHeight - cfgs.forceDownMax
        : gHeight - (hedge["下"] || 0);
    const gLeftMax =
      cfgs.forceLeftMax != null ? cfgs.forceLeftMax : hedge["左"] || 0;
    const gRightMax =
      cfgs.forceRightMax != null
        ? gWidth - cfgs.forceRightMax
        : gWidth - (hedge["右"] || 0);
  ...
  function createTextSprite(text, x, y, textStyle, fontData = {}, handler) {
    ...
    // 有効でないなら非表示
    if (textStyle["可"] != null && !textStyle["可"]) {
    ...
    lines.y = lines.y.map(
      (y) => y + textSprite.y - textTotalH / 2 + lines.h[0] / 2
    );
    // フォントスタイル取得
  ...
  function scanControlChar(text, fontSize, oriFonts, bitmap = null) {
    ...
    match = text.match(/^\\c\[([^\]]*)\]/);
    if (match) {
      let common = $gameSystem._textCommonKe
        ? $gameSystem._textCommonKe[match[1]] || ""
        : "";
  ...
  function createBaseSprite(
    ...
    // 有効でないなら非表示
    if (baseStyle["可"] != null && !baseStyle["可"]) {
    ...
    if (tsuno) {
      ...
      if (cfgs.rel == "up") {
        ...
        tsunoSprite.y =
          baseSprite.y + baseBitmap.height / 2 + tsunoBitmap.height / 2 - outH;
      } else if (cfgs.rel == "down") {
        tsunoOy = tsunoBitmap.height;
        tsunoSprite.y =
          baseSprite.y - baseBitmap.height / 2 - tsunoBitmap.height / 2 + outH;
  ...
  function drawBitmapByStyle(bitmap, x, y, width, height, style, rel = null) {
    ...
    if (form == "スクエア") {
      ...
      if (outH) {
        strokeShout(bitmap, x, y, width, height, outColor, outH, shape);
      }
    } else if (form == "ダイヤ") {
  ...
  function designShout(bitmap, x, y, width, height, shape = 1) {
    ...
    if (!shape) {
      ...
      for (let i = 0; i < xNum - 1; i++) {
        rand = outs.length
          ? outs.shift()
          : yOut * r1 + Math.randomInt(yOut * r2);
      ...
      for (let i = 0; i < yNum - 1; i++) {
        rand = outs.length
          ? outs.shift()
          : xOut * r1 + Math.randomInt(xOut * r2);
