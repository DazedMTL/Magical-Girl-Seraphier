...
(function () {
  ...
  const settings = {
    enableOnlyWhenSolo:
      String(pluginParameters["Enable Only When Solo Party"] || "true") ===
      "true",
    skipSelectTargetInMenu:
      String(
        pluginParameters["Skip Select Item And Skill Target In Menu"] || "true"
      ) === "true",
    skipSelectTargetInBattle:
      String(
        pluginParameters[
          "Skip Select Item And SKill Target For Friend In Battle"
        ]
      ) === "true",
  };
  ...
  Scene_ItemBase.prototype.skipSelectTarget = function () {
    return (
      settings.skipSelectTargetInMenu &&
      (!settings.enableOnlyWhenSolo || $gameParty.isSoloParty())
    );
  };
  ...
  Scene_Battle.prototype.skipSelectTarget = function () {
    return (
      settings.skipSelectTargetInBattle &&
      (!settings.enableOnlyWhenSolo || $gameParty.isSoloParty())
    );
  };

  const _Scene_Battle_selectActorSelection =
