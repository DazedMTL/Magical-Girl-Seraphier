...
(() => {
  ...
  Game_Map.prototype.updatePointScroll = function () {
    if (this.isPointScrolling()) {
      ...
      let x =
        tx + ((ox - tx) * duration) / max - $gameMap.screenTileX() / 2 + 0.5;
      let y =
        ty + ((oy - ty) * duration) / max - $gameMap.screenTileY() / 2 + 0.5;
      x = $gameMap.isLoopHorizontal()
        ? (x + $gameMap.width()) % $gameMap.width()
        : Math.max(x, 0);
      y = $gameMap.isLoopVertical()
        ? (y + $gameMap.height()) % $gameMap.height()
        : Math.max(y, 0);
      this._displayX = x;
      this._displayY = y;
    }
