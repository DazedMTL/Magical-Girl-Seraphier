...
(() => {
  ...
  function nuun_GausePlugins() {
    return (
      ...
      Imported.NUUN_BattlerMPGauge ||
      Imported.NUUN_BattlerTPGauge ||
      Imported.NUUN_BattlerTpbGauge
    );
  }
  ...
  Bitmap.prototype.nuun_contentsBackBlt = function (
    source,
    sx,
    sy,
    sw,
    ...
    const x = Math.floor(
      !mode && scaleMode ? 0 : ((dw - source.width) / 2) * -1 * scale
    );
    const y = Math.floor(
      !mode
        ? 0
        : scaleMode
        ? ((dh - source.height) / 2) * -1 * scale2
        : ((dh - source.height) / 2) * -1 * scale
    );
