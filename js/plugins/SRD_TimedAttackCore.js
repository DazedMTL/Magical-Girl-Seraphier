...
(function (_) {
  ...
  _.allowTouch =
    String(params["Enable Touch?"]).trim().toLowerCase() === "true";
  ...
  _.activateCondition = _.allowTouch
  ...
  _.organizeInfo = function (o) {
    if (o.type === _.TAS_ID) {
      ...
      if (o.info.match(/Cursor[ ]?Image:\s*(.*)/im))
        o.image = String(RegExp.$1);
      if (o.info.match(/Background[ ]?Image:\s*(.*)/im))
        o.bi = String(RegExp.$1);
      if (o.info.match(/Window[ ]?Opacity:\s*(.*)/im))
        o.opacity = parseInt(RegExp.$1);
      if (o.info.match(/Target[ ]?Location:\s*(.*)/im))
        o.target = String(RegExp.$1);
      if (o.info.match(/Repeat[ ]?Type:\s*(.*)/im))
        o.rt = String(RegExp.$1).trim().toLowerCase();
      if (o.info.match(/Speed:\s*(.*)/im)) o.speed = String(RegExp.$1);
      if (o.info.match(/Main[ ]?Color:\s*(.*)/im)) o.color = String(RegExp.$1);
      if (o.info.match(/Shape:\s*(.*)/im))
        o.shape = String(RegExp.$1).trim().toLowerCase();
      if (o.info.match(/Width:\s*(.*)/im)) o.width = parseInt(RegExp.$1);
      if (o.info.match(/Outline[ ]?Color:\s*(.*)/im))
        o.outline = String(RegExp.$1);
      if (o.info.match(/Outline[ ]?Size:\s*(.*)/im))
        o.size = parseInt(RegExp.$1);
      if (o.info.match(/Direction:\s*(.*)/im))
        o.direction = String(RegExp.$1).trim().toLowerCase();
      if (o.info.match(/Flash[ ]?Rate:\s*(.*)/im))
        o.flash = parseInt(RegExp.$1);
  ...
  _.notDefault = function () {
    return (
      _.default === "actor" || _.default === "class" || _.default === "weapon"
    );
  };
  ...
  Scene_Battle.prototype.createAllWindows = function () {
    ...
    else if (_.xAlign === "right")
      this._tasAttack._window.x =
        Graphics.width - this._tasAttack._window.width;
    else
      this._tasAttack._window.x =
        Graphics.width / 2 - this._tasAttack._window.width / 2;

    if (_.yAlign === "top") this._tasAttack._window.y = 0;
    else if (_.yAlign === "bottom")
      this._tasAttack._window.y =
        Graphics.height - this._tasAttack._window.height;
    else
      this._tasAttack._window.y =
        Graphics.height / 2 - this._tasAttack._window.height / 2;
  ...
  BattleManager.startAction = function () {
    ...
    if (
      ...
      this._subject.currentAction().makeTargets()
    ) {
  ...
  if (Imported.YEP_BattleEngineCore) {
    ...
    BattleManager.updatePhase = function () {
      if (
        !this._action.item().meta.isUsingTimedAttackActionSequences &&
        this.checkIfTASActionAllowed() &&
        (this._action.item().meta["SRD TAS"] || _.notDefault()) &&
        this[_.pass + "Allow TAS"][0]
      ) {
        if (this[_.pass + "Allow TAS"][1]) {
          if (
            this._action.item().meta["SRD TAS"].type === "actor" ||
            (_.default === "actor" && !this._action.item().meta["SRD TAS"])
          ) {
            if (this._subject.actor().meta["SRD TAS"]) {
              this._tasAttackFrameWindow.setItem(
                this._subject.actor().meta["SRD TAS"]
              );
            ...
            this._action.item().meta["SRD TAS"].type === "class" ||
            (_.default === "class" && !this._action.item().meta["SRD TAS"])
          ) {
            if (
              this._subject.currentClass() &&
              this._subject.currentClass().meta["SRD TAS"]
            ) {
              this._tasAttackFrameWindow.setItem(
                this._subject.currentClass().meta["SRD TAS"]
              );
            ...
            this._action.item().meta["SRD TAS"].type === "weapon" ||
            (_.default === "weapon" && !this._action.item().meta["SRD TAS"])
          ) {
            if (
              this._subject.weapons()[0] &&
              this._subject.weapons()[0].meta["SRD TAS"]
            ) {
              this._tasAttackFrameWindow.setItem(
                this._subject.weapons()[0].meta["SRD TAS"]
              );
            ...
            this._tasAttackFrameWindow.setItem(
              this._action.item().meta["SRD TAS"]
            );
            this._tasAttackFrameWindow.open();
            this._tasAttackFrameWindow.start();
            this[_.pass + "Allow TAS"][1] = false;
        ...
        if (this._subject.isActor() && !this[_.pass + "Boolean 1"]) {
          ...
          this._logWindow.startAction(
            this[_.pass + "Temp Subject"],
            this[_.pass + "Temp Action"],
  ...
  var _Window_BattleLog_showAnimation =
  ...
  TimedAttackSystem.prototype.fittingHeight = function (numLines) {
    return (
      numLines * this.lineHeight() +
      Window_Base.prototype.standardPadding.call(this) * 2
    );
  };
  ...
  TimedAttackSystem.prototype.playDefaultGame = function () {
    ...
    if (this._notPressed) {
      ...
      if (
        this._xPosition > this.windowWidth() ||
        this._xPosition < -this._item.width
      ) {
        if (this._rt === "repeat") {
    ...
    this._window.contents.blt(
      ...
      0,
      0,
      this._window.contentsWidth(),
      this._window.contentsHeight()
    );
    if (this._flashTime % this._flash < Math.floor(this._flash / 2)) {
      if (this._shape === "image" && this._image != undefined) {
        ...
        this._window.contents.blt(
          ...
          bitmap.width,
          bitmap.height,
          this._xPosition - bitmap.width / 2,
          0
        );
      } else if (this._shape === "rectangle") {
        this.contents.fillRect(
          this._xPosition - this._item.width / 2,
          0,
          this._item.width,
          this.height - this._window.standardPadding() * 2,
          this._item.outline
        );
        this.contents.fillRect(
          this._xPosition + this._size - this._item.width / 2,
          0 + this._size,
          this._item.width - this._size * 2,
          this.height - this._window.standardPadding() * 2 - this._size * 2,
          this._item.color
        );
      } else if (this._shape === "oval") {
        var height = this.height - this._window.standardPadding() * 2;
        this._window.contents.drawCircle(
          this._xPosition - this._item.width / 2,
          height / 2,
          this._item.width,
          this._item.outline
        ...
        this._window.contents.drawCircle(
          this._xPosition + this._size / 2 - 1 - this._item.width / 2,
          height / 2 + this._size / 2 - 1,
          this._item.width - this._size,
          this._item.color
        );
      }
    }
