...
(function () {
  Galv.NEWGP.newGameTxt =
    PluginManager.parameters("Galv_NewGamePlus")["Command Text"];
  Galv.NEWGP.helpTxt =
    PluginManager.parameters("Galv_NewGamePlus")["Help Text"];
  Galv.NEWGP.icon = Number(
    PluginManager.parameters("Galv_NewGamePlus")["Active Icon"]
  );
  Galv.NEWGP.iconNGP = Number(
    PluginManager.parameters("Galv_NewGamePlus")["NGP Icon"]
  );
  Galv.NEWGP.maxGold = Number(
    PluginManager.parameters("Galv_NewGamePlus")["Max Gold"]
  );

  var txt =
    PluginManager.parameters("Galv_NewGamePlus")["Active Icon XY"].split(",");
  Galv.NEWGP.iconXY = [Number(txt[0]), Number(txt[1])];

  var txt =
    PluginManager.parameters("Galv_NewGamePlus")["NGP Icon XY"].split(",");
  ...
  Galv.NEWGP.varType = Number(
    PluginManager.parameters("Galv_NewGamePlus")["Variable Type"]
  );
  Galv.NEWGP.varList = Galv.NEWGP.makeList(
    PluginManager.parameters("Galv_NewGamePlus")["Variable List"]
  );

  Galv.NEWGP.swiType = Number(
    PluginManager.parameters("Galv_NewGamePlus")["Switch Type"]
  );
  Galv.NEWGP.swiList = Galv.NEWGP.makeList(
    PluginManager.parameters("Galv_NewGamePlus")["Switch List"]
  );

  Galv.NEWGP.itemType = Number(
    PluginManager.parameters("Galv_NewGamePlus")["Item Type"]
  );
  Galv.NEWGP.itemList = Galv.NEWGP.makeList(
    PluginManager.parameters("Galv_NewGamePlus")["Item List"]
  );

  Galv.NEWGP.weaponType = Number(
    PluginManager.parameters("Galv_NewGamePlus")["Weapon Type"]
  );
  Galv.NEWGP.weaponList = Galv.NEWGP.makeList(
    PluginManager.parameters("Galv_NewGamePlus")["Weapon List"]
  );

  Galv.NEWGP.armorType = Number(
    PluginManager.parameters("Galv_NewGamePlus")["Armor Type"]
  );
  Galv.NEWGP.armorList = Galv.NEWGP.makeList(
    PluginManager.parameters("Galv_NewGamePlus")["Armor List"]
  );

  Galv.NEWGP.actorList = Galv.NEWGP.makeList(
    PluginManager.parameters("Galv_NewGamePlus")["Actor List"]
  );
  ...
  Galv.NEWGP.setupNewGamePlus = function () {
    ...
    // GAME PARTY
    var maxGold =
      Galv.NEWGP.maxGold < 0 ? game.Party._gold : Galv.NEWGP.maxGold;
  ...
  Galv.NEWGP.setVariables = function (game) {
    if (Galv.NEWGP.varType == 0) {
      // Only transfer List
      for (var i = 0; i < Galv.NEWGP.varList.ids.length; i++) {
        // constants
        $gameVariables._data[Galv.NEWGP.varList.ids[i]] =
          game.Variables._data[Galv.NEWGP.varList.ids[i]];
      }
      for (var i = 0; i < Galv.NEWGP.varList.ranges.length; i++) {
        // ranges
    ...
    } else if (Galv.NEWGP.varType == 1) {
      // Transfer all EXCEPT list
  ...
  Galv.NEWGP.setSwitches = function (game) {
    if (Galv.NEWGP.swiType == 0) {
      // Only transfer List
      for (var i = 0; i < Galv.NEWGP.swiList.ids.length; i++) {
        // constants
        // change all constant variables
        $gameSwitches._data[Galv.NEWGP.swiList.ids[i]] =
          game.Switches._data[Galv.NEWGP.swiList.ids[i]];
      }
      for (var i = 0; i < Galv.NEWGP.swiList.ranges.length; i++) {
        // ranages
  ...
  Galv.NEWGP.setActors = function (game) {
    for (var i = 0; i < Galv.NEWGP.actorList.ids.length; i++) {
      // constants
      $gameActors._data[Galv.NEWGP.actorList.ids[i]] =
        game.Actors._data[Galv.NEWGP.actorList.ids[i]];
    }
    for (var i = 0; i < Galv.NEWGP.actorList.ranges.length; i++) {
      // ranges
  ...
  Galv.NEWGP.setItems = function (game, iType) {
    ...
    if (type == 0) {
      ...
      for (var i = 0; i < list.ranges.length; i++) {
        // ranges
      ...
      for (var i = 0; i < list.ids.length; i++) {
        ...
        var itemId = list.ids[i];
        if (itemAmount)
          $gameParty.gainItem(itemData[itemId], -game.Party[itemBox][itemId]);
      }
      for (var i = 0; i < list.ranges.length; i++) {
        // ranges
        var range = list.ranges[i];
  ...
  Galv.NEWGP.isReplay = function (a, b) {
    if (!a && !b) {
      ...
      if (b < 0) {
        ...
        return (
          $gameSystem.newGamePlus > array[0] &&
          $gameSystem.newGamePlus < array[1]
        );
      }
    }
  };
  ...
  Scene_Title.prototype.create = function () {
    ...
    this._commandWindow.setHandler(
      "newGamePlus",
      this.commandNewGamePlus.bind(this)
    );
  };
  ...
  Galv.NEWGP.Window_TitleCommand_makeCommandList =
  ...
  Galv.NEWGP.Window_SavefileList_drawContents =
  ...
  Window_SavefileList.prototype.drawNewGamePlus = function (info, x, y, width) {
    if (info.gameCompleted) {
      ...
      this.drawIcon(
        Galv.NEWGP.icon,
        x + Galv.NEWGP.iconXY[0],
        y + Galv.NEWGP.iconXY[1]
      );
    }
    ...
    if (NGPnumber > 0) {
      ...
      this.drawIcon(
        Galv.NEWGP.iconNGP,
        x + Galv.NEWGP.iconNGPXY[0],
        y + Galv.NEWGP.iconNGPXY[1]
      );
      // draw number
      if (NGPnumber > 1) {
        ...
        this.drawText(
          NGPnumber,
          x + Galv.NEWGP.iconNGPXY[0] + 3,
          y + Galv.NEWGP.iconNGPXY[1] + 10,
