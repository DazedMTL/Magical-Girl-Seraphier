...
(function () {
  ...
  Game_Actor.prototype.isSealCommand = function (commandNames) {
    const switchId = this.getSealMetaInfo([
      commandNames[0] + "SealSwitch",
      commandNames[1] + "封印スイッチ",
    ]);
    if (switchId && (switchId === true || $gameSwitches.value(switchId))) {
      return true;
    }
    const formula = this.getSealMetaInfo([
      commandNames[0] + "SealFormula",
      commandNames[1] + "封印計算式",
    ]);
  ...
  const _Window_ActorCommand_addGuardCommand =
  ...
  const _Window_ActorCommand_addItemCommand =
  ...
  const _Scene_Battle_selectNextCommand =
