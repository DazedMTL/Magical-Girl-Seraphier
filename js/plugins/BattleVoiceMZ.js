...
(() => {
  ...
  PluginManager.registerCommand(pluginName, "stateChange", (args) => {
    const voiceToAdd = join2voices(
      args.voiceAtStateAdded1,
      args.voiceAtStateAdded2
    );
    const voiceToRemove = join2voices(
      args.voiceAtStateRemoved1,
  ...
  const _voiceFileName = (setVoice, battlerMeta, type) => {
    ...
    switch (type) {
      ...
      case "friendmagic":
        name = split(
          a.friendMagic || m.friendMagicVoice || a.magic || m.magicVoice
        );
        break;
      case "magic":
  ...
  const preemptiveFirstVoice = () => {
    if (BattleManager._preemptive) {
      const candidates = $gameParty.aliveBattleMembers().filter((actor) => {
        return (
          actor.actor().meta.preemptiveFirstVoice != null ||
  ...
  const surpriseFirstVoice = () => {
    if (BattleManager._surprise) {
      const candidates = $gameParty.aliveBattleMembers().filter((actor) => {
        return (
          actor.actor().meta.surpriseFirstVoice != null ||
  ...
  const normalFirstVoice = () => {
    if (!BattleManager._surprise) {
      const candidates = $gameParty.aliveBattleMembers().filter((actor) => {
        return (
          actor.actor().meta.firstVoice != null ||
