...
Game_Live2d.prototype.ReflectSavedataToModels = function () {
  ...
  L2DINmodels.forEach(function (data) {
    ...
    if (saveobj.vis) {
      ...
      $gameLive2d.motionGroup[i] = saveobj.motionGroup;
      $gameLive2d.motionNumber[i] = saveobj.motionNumber;
      $gameLive2d.motionLoop[i] = saveobj.motionLoop;
      ...
      if ($gameParty.members()[i]._name == $gameLive2d._name[number]) {
        for (var j = 0; j < 5; j++) {
          ...
          if ($gameLive2d.linkEquip_None[number][j] != "") {
            ...
            var modelNo = Live2DManager.getNumberFromName(
              $gameLive2d._name[number]
            );
...
Sprite_Live2d.prototype.loadModel = function (name) {
  ...
  PIXI.live2d.Live2DModel.from("./" + name, option)
    .then((model) => {
      this._model = model;
      this.addChild(model);
...
(() => {
  ...
  Scene_Base.prototype.createlive2d = function () {
    ...
    for (var i = 1; i <= $gameLive2d.MAXNUMBER; i++) {
      var fileFullPath =
        $gameLive2d._folder[i] + $gameLive2d._model[i] + ".model3.json";
  ...
  Scene_Base.prototype.updatelive2d = function () {
    for (var i = 1; i <= $gameLive2d.MAXNUMBER; i++) {
      ...
      if (this.live2dSprite[i]._model) {
        ...
        this.live2dSprite[i].x = $gameLive2d.pos_x[i];
        this.live2dSprite[i].y = $gameLive2d.pos_y[i];
        this.live2dSprite[i].scale.set(
          0.2 * $gameLive2d.scale[i],
          0.2 * $gameLive2d.scale[i]
        );
        ...
        if (this.live2dSprite[i]._model.internalModel) {
          ...
          if (
            this.live2dSprite[
              i
            ]._model.internalModel.motionManager.isFinished() ||
            $gameLive2d.IsForcedExe_NextMotion[i] == true
          ) {
            ...
            if (motionElement) {
              ...
              if ($gameLive2d.paraminitskip[i]) {
                this.live2dSprite[
                  i
                ]._model.internalModel.coreModel._parameterValues.fill(0);
              }
              ...
              this.live2dSprite[
                i
              ]._model.internalModel.motionManager.motionGroups[
                motionElement.groupName
              ][motionElement.number].setIsLoop(motionElement.Isloop);

              this.live2dSprite[i]._model.motion(
                motionElement.groupName,
                motionElement.number,
                3
              );
          ...
          if ($gameLive2d.IsForcedExe_ChangeExpression[i] == true) {
            var expressions =
              this.live2dSprite[i]._model.internalModel.settings.expressions;
            var expressionId = 0;
  ...
  PluginManager.registerCommand(pluginName, "motion", (args) => {
    ...
    var innerMotionName = $gameLive2d.InnerMotionName(
      args.modelName,
      args.motionName
    );
