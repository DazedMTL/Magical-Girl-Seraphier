 ...
 *   Version 1.01  2021/09/03  RPGツクールMZに仮対応
 * ------------------------------------------------------------------------------
...
(function () {
...

  {
    ...
    Scene_Title.prototype.create = function () {
      ...
      let extension =
        _plugin.extension === "auto"
          ? Game_Interpreter.prototype.videoFileExt.call(null)
          : _plugin.extension;
      let sprite = new Sprite_Video(
        "./movies/" + _plugin.file_name + extension,
        true
      );
      sprite.x = _plugin.x;
      sprite.y = _plugin.y;
  ...
  class Sprite_Video extends Sprite {
    ...
    setVideo(source, loop) {
      ...
      video.addEventListener("loadeddata", (e) => {
        ...
        bitmap.addLoadListener(() => {
          if (this.bitmap) {
            let resource =
              this.bitmap.baseTexture.resource || this.bitmap.baseTexture;
            resource.source.pause();
          }
          video.volume = this._volume;
          if (PIXI.VideoBaseTexture) {
            let texture = (bitmap.__baseTexture =
              PIXI.VideoBaseTexture.fromVideo(video));
            texture.autoPlay = false;
          } else {
            let texture = (bitmap._baseTexture =
              PIXI.Texture.from(video).baseTexture);
            texture.resource.autoPlay = false;
          }
          if (this.parent) {
            video.play();
            if (video.paused) {
    ...
    updateVideo() {
      if (this.bitmap) {
        let resource =
          this.bitmap.baseTexture.resource || this.bitmap.baseTexture;
        if (!resource.source.paused) {
          if (this.bitmap._setDirty) {
    ...
    updateVolume() {
      ...
      if (this.bitmap) {
        let resource =
          this.bitmap.baseTexture.resource || this.bitmap.baseTexture;
        resource.source.volume = this._volume * this._fade_volume;
      }
    }
  }
})();
