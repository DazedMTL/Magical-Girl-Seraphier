...
(function () {
  ...
  Game_Actor.prototype.releaseUnequippableItems = function (forcing) {
    ...
    if (!equalSlots(this._oldSlots, slots)) {
      ...
      for (
        let equipType = 1;
        equipType < $dataSystem.equipTypes.length;
        equipType++
      ) {
      ...
      for (
        ...
        const oldSlotTypeCount = this._oldSlots.filter(
          (s) => s == equipType
        ).length;
      ...
      for (const equip of this._equips) {
        if (equip.object()) {
          ...
          const newItemCount = newEquips
            .concat(removeEquips)
            .filter((e) => e.object() == equip.object()).length;
          const oldItemCount = this._equips.filter(
            (e) => e.object() == equip.object()
          ).length;
  ...
  if (pPagingEquipmentType) {
    ...
    Window_EquipSlot.prototype.pageItemsCount = function (pageNo) {
      ...
      // 空のページは除去
      const filterPageSlots = pageSlots.filter(
        (pageSlot) => pageSlot.length > 0
      );
  ...
  if (pStatusShowSlots) {
    ...
    Window_StatusEquip.prototype.maxVisibleItems = function () {
      ...
      const filterSlots = slots.filter((slot) =>
        pStatusShowSlots.includes(String(slot))
      );
