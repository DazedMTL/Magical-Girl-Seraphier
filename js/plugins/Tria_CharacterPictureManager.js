...
(function () {
  ...
  class StandPictureParam {
    setup(actor, scene, index) {
      ...
      this._actor = actor;
      this._standPictures = param.PictureList.filter(
        (picture) => picture.ActorId === actor.actorId()
      );
      if (this._standPictures.length <= 0) {
        return false;
      }
      this._standPictures.forEach((picture) =>
        this.setupPosition(picture, scene, index)
      );
    ...
    updatePictureFiles() {
      this._standPictures.forEach((picture) => {
        ...
        picture.FileList.clone()
          .reverse()
          .some((file) => this.setFileNameIfValid(file, picture));
        if (!picture.FileName && picture.DynamicFileName) {
          picture.FileName = this.createDynamicFileName(
            picture.DynamicFileName
          );
        }
      });
    ...
    setFileNameIfValid(file, picture) {
      ...
      const conditions = [];
      conditions.push(
        () => !file.HpUpperLimit || file.HpUpperLimit >= a.hpRate() * 100
      );
      conditions.push(
        () => !file.HpLowerLimit || file.HpLowerLimit <= a.hpRate() * 100
      );
      ...
      conditions.push(
        () => !file.Weapon || a.hasWeapon($dataWeapons[file.Weapon])
      );
      conditions.push(() => !file.Armor || a.hasArmor($dataArmors[file.Armor]));
      conditions.push(
        () => !file.Note || this.findStandPictureMeta() === file.Note
      );
    ...
    findActorState() {
      const state = this._actor
        .states()
        .filter((state) => !state.meta.NoStandPicture)[0];
  ...
  Scene_Base.prototype.createAllStandPicture = function () {
    ...
    this._standSpriteScene = param.SceneList.filter(
  ...
  Scene_Base.prototype.updateStandPicture = function (actor, index) {
    ...
    const sprite = usePointAdjust
      ? new Sprite_StandPictureWithDrag(pictureParam)
  ...
  class Sprite_StandPicture extends Sprite {
    ...
    setup(pictureParam) {
      ...
      this._pictures
        .updatePictureFiles()
        .forEach((picture) => this.addChild(this.createChild(picture)));
      this.updatePosition();
    }
  ...
  class Sprite_StandPictureWithDrag extends Sprite_StandPicture {
    ...
    update() {
      super.update();
      const children = Input.isPressed("control")
        ? this.children
        : this.children.clone().reverse();
  ...
  class Sprite_StandPictureChildWithDrag extends Sprite_StandPictureChild {
    ...
    updateDrag() {
      ...
      Graphics.drawPositionInfo(
        `BaseX:${this.parent.x} BaseY:${this.parent.y} PictureX:${this.x} PictureY:${this.y} `
      );
