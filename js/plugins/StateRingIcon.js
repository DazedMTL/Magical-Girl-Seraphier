...
(() => {
  ...
  Game_BattlerBase.prototype.getStateTurns = function () {
    const stateTurns = this.states().map(function (state) {
      if (state.iconIndex <= 0) {
        ...
        return (
          Math.ceil(this._stateTurns[state.id]) +
          (state.autoRemovalTiming === 1 ? 1 : 0)
        );
      }
    }, this);
  ...
  Game_BattlerBase.prototype.getAllTurns = function () {
    return this.getStateTurns()
      .concat(this.getBuffTurns())
      .map(function (turn) {
  ...
  if (param.ActorRingIcon) {
    const _Sprite_Actor_createStateSprite =
    ...
    Sprite_Actor.prototype.updateStateSprite = function () {
      this._stateIconSprite.y -= Math.round(
        (this._mainSprite.height + 40) * 0.9
      );
  ...
  Sprite_StateIcon.prototype.updateRingIconChild = function () {
    if (
      this._iconsSprites.length > param.LineViewLimit &&
      param.LineViewLimit > 0
    ) {
  ...
  Sprite_StateIcon.prototype.getIconRadian = function (index) {
    let radian =
      (this._animationCount / this.getCycleDuration() +
        index / this._iconsSprites.length) *
      Math.PI *
      2;
