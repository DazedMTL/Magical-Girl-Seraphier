...
(function () {
        ...
        .filter(function (_a) {
          ...
          if (state.timer < state.duration) {
            ...
            Object.keys(state.finishParams).forEach(function (key) {
              tween.target[key] =
                state.startParams[key] +
                (state.finishParams[key] - state.startParams[key]) * n_1;
            });
          } else {
            Object.keys(state.finishParams).forEach(function (key) {
          ...
          var result =
            state.timer < state.duration || _this._beginAnimation(tween, state);
  ...
  (() => {
    ...
    SceneManager.updateScene = function () {
      const isStarted =
        this._scene && this._scene.isStarted() && this.isGameActive();
