...
(() => {
  ...
  PluginManagerEx.registerCommand(script, "MAKE", function (args) {
    ...
    $gameMap.spawnEvent(
      this.getEventIdForEventReSpawn(args.id, template),
      args.x,
      args.y,
      template
    );
  ...
  Game_Map.prototype.spawnEvent = function (originalEventId, x, y, isTemplate) {
    if (
      ...
      const event = new Game_PrefabEvent(
        this._mapId,
        eventId,
        originalEventId,
        x,
  ...
  Game_Map.prototype.spawnEventRandom = function (
    ...
    if (conditionMap.overlap) {
      conditions.push(
        this.isErsCheckCollidedSomeone.bind(this, conditionMap.overlap)
      );
    }
    if (conditionMap.terrainTags && conditionMap.terrainTags.length > 0) {
      conditions.push(
        this.isErsCheckTerrainTag.bind(this, conditionMap.terrainTags)
      );
    }
  ...
  Game_Map.prototype.getConditionalValidPosition = function (
    conditions,
    algorithm
  ) {
    if (algorithm === 0) {
      ...
      const positions = [];
      for (let ix = 0; ix < $dataMap.width; ++ix)
        for (let iy = 0; iy < $dataMap.height; ++iy) {
          if (conditions.every(this.checkValidPosition.bind(this, ix, iy))) {
            positions.push({ x: ix, y: iy });
          }
        }
      return positions.length
        ? positions[Math.randomInt(positions.length)]
        : null;
    }
  };
  ...
  Spriteset_Map.prototype.refresh_event_text_field = function () {
    for (let i = 0; i < this._characterSprites.length; i++) {
      if (!this._sprite_char_text[i]) {
        this._sprite_char_text[i] = new Sprite_CharText(
  ...
  if (typeof Window_EventMiniLabel !== "undefined") {
    const _Window_EventMiniLabel_gatherDisplayData =
      Window_EventMiniLabel.prototype.gatherDisplayData;
