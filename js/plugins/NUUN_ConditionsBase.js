...
(() => {
  ...
  const TriggerConditions =
    (NUUN_Base_Ver >= 113
      ? DataManager.nuun_structureData(parameters["TriggerConditions"])
      : null) || [];
  ...
  Game_Action.prototype.traitTriggerConditions = function (
    target,
    tag1,
    tag2,
    tag3,
    ...
    return subject.getTraitTriggerConditions(
      target,
      tag1,
      tag2,
      tag3,
  ...
  Game_Action.prototype.triggerConditions = function (
    obj,
    target,
    tag1,
    tag2,
    ...
    //単体
    const subject = this.subject();
    return subject.getTriggerConditions(
      obj,
      target,
      tag1,
      tag2,
  ...
  Game_BattlerBase.prototype.getTraitTriggerConditions = function (
    target,
    tag1,
    tag2,
    tag3,
  ...
  Game_BattlerBase.prototype.getTriggerConditions = function (
    obj,
    target,
    tag1,
    tag2,
  ...
  Game_BattlerBase.prototype.getTriggerConditionsParams = function (
    obj,
    target,
    params,
    action,
    ...
    //単体
    return getTriggerConditionsParams(
      obj,
      this,
      target,
      params,
  ...
  function getTriggerConditions(
    ...
    if (getTriggerConditionsMeta(obj, tag1, tag2, tag3, tag4)) {
      const result1 = getTriggerConditionsResult(
        obj,
        subject,
        tag1,
        "Subject",
      ...
      const result2 = getTriggerConditionsResult(
        obj,
        target,
        tag2,
        "Target",
      ...
      const result3 = getTriggerConditionsResult(
        obj,
        null,
        tag3,
        "Party",
      ...
      const result4 = getTriggerConditionsResult(
        obj,
        null,
        tag4,
        "Troop",
  ...
  function getTriggerConditionsParams(
    obj,
    subject,
    target,
    params,
    ...
    if (getTriggerConditionsParam(params)) {
      const result1 = getTriggerConditionsParamsResult(
        obj,
        subject,
        params.SubjectCond,
        "Subject",
      ...
      const result2 = getTriggerConditionsParamsResult(
        obj,
        target,
        params.TargetCond,
        "Target",
      ...
      const result3 = getTriggerConditionsParamsResult(
        obj,
        null,
        params.PartyCond,
        "Party",
      ...
      const result4 = getTriggerConditionsParamsResult(
        obj,
        null,
        params.TroopCond,
        "Troop",
  ...
  function getTriggerConditionsResult(
    obj,
    target,
    tag,
    mode,
  ...
  function getTriggerConditionsParamsResult(
    obj,
    target,
    param,
    mode,
  ...
  function isTriggerConditionsSome(list, target, mode, action, damage) {
    return list.some((id) =>
      triggerConditions(TriggerConditions[id - 1], target, mode, action, damage)
    );
  }
  ...
  function memberTriggerConditions(data, target, mode) {
    ...
    if (data.MemberConditionsType === "BattleMember") {
      ...
      Imported.NUUN_SupportActor &&
      data.MemberConditionsType === "SupportMember"
    ) {
      ...
      if (mode === "Party") {
        return unit
          ...
          .some((member) =>
            conditionsRate(
              data,
              member.currentExp() - member.currentLevelExp(),
              member.nextLevelExp() - member.currentLevelExp()
            )
          );
      } else if (target && target.isActor()) {
        return conditionsRate(
          data,
          target.currentExp() - target.currentLevelExp(),
          target.nextLevelExp() - target.currentLevelExp()
        );
      }
      return false;
    }
  ...
  function paramTriggerConditions(data, target, mode) {
    ...
    if (mode === "Party" || mode === "Troop") {
      return unit
        .members()
        .some((member) => member.triggerParamConditions(data));
    } else {
  ...
  function equipTriggerConditions(data, target, mode) {
    ...
    if (data.EquipConditionsType === "Weapon") {
      if (mode === "Party") {
        return unit
          .members()
          .some((member) => equipWeapon(data.IDList, member));
      } else {
      ...
      if (mode === "Party") {
        return unit
          .members()
          .some((member) => equipWeaponType(data.IDList, member));
      } else {
      ...
      if (mode === "Party") {
        return unit
          .members()
          .some((member) => equipArmorType(data.IDList, member));
      } else {
  ...
  function useItemSkillTriggerConditions(data, target, mode, action) {
    ...
    if (data.ItemSkillConditionsType === "Item") {
      ...
      return action.isSkill()
        ? useSkillType(data.IDList, action.item())
        : false;
    }
  }
  ...
  function variableTriggerConditions(data) {
    if (data.VariableConditionsType === "Var") {
      return getValList(data.IDList).some((listId) =>
        conditionsNum(data, $gameVariables.value(listId))
      );
    }
    return false;
  }
  ...
  function switchTriggerConditions(data) {
    if (data.SwitchConditionsType === "ONSwitch") {
      return getValList(data.IDList).some((listId) =>
        $gameSwitches.value(listId)
      );
    } else if (data.SwitchConditionsType === "OFFSwitch") {
      return getValList(data.IDList).some(
        (listId) => !$gameSwitches.value(listId)
      );
    }
    return false;
  }
  ...
  function conditionsNum(data, num) {
    ...
    return (
      num >= data.DwLimit && (data.UpLimit > 0 ? num <= data.UpLimit : true)
    );
  }
  ...
  function conditionsRate(data, num, paramMax) {
    ...
    return (
      num >= (paramMax * data.DwLimit) / 100 &&
      (data.UpLimit > 0 ? num <= (paramMax * data.UpLimit) / 100 : true)
    );
  }
  ...
  function conditionsAve(data, unit) {
    ...
    const sum = members.reduce(
      (r, member) => r + (member._level ? member._level : 0),
      0
    );
  ...
  function state(data, member) {
    ...
    return list.some(
      (id) => id > 0 && stateTurn(data, getStateId(id, states), member)
    );
  }
  ...
  function stateTurnCount(data, member) {
    ...
    const list = getValList(data.IDList);
    return list.some(
      (id) => id > 0 && getStateTurnCount(data, getStateId(id, states), member)
    );
  }
  ...
  function possessionItems(data) {
    return getValList(data.IDList).some((listId) =>
      conditionsNum(data, $gameParty.numItems($dataItems[listId]))
    );
  }

  function possessionWeapons(data) {
    return getValList(data.IDList).some((listId) =>
      conditionsNum(data, $gameParty.numItems($dataWeapons[listId]))
    );
  }

  function possessionArmors(data) {
    return getValList(data.IDList).some((listId) =>
      conditionsNum(data, $gameParty.numItems($dataArmors[listId]))
    );
  }

  function notPossessionItems(data) {
    return !getValList(data.IDList).some((listId) =>
      $gameParty.hasItem($dataItems[listId])
    );
  }

  function notPossessionWeapons(data) {
    return !getValList(data.IDList).some((listId) =>
      $gameParty.hasItem($dataWeapons[listId])
    );
  }

  function notPossessionArmors(data) {
    return !getValList(data.IDList).some((listId) =>
      $gameParty.hasItem($dataArmors[listId])
    );
  }

  function masterSkill(data, member) {
    return getValList(data.IDList).some((listId) =>
      member.skills().some((skill) => skill.id === listId)
    );
  }

  function masterEnemySkill(data, member) {
    return getValList(data.IDList).some((listId) =>
      member.enemy().actions.some((action) => action.skillId === listId)
    );
  }
  ...
  function equipWeapon(idList, member) {
    if (idList == 0) {
      ...
      return getValList(idList).some((listId) =>
        member.hasWeapon($dataWeapons[listId])
      );
    }
  }
  ...
  function equipArmor(idList, member) {
    if (idList == 0) {
      ...
      return getValList(idList).some((listId) =>
        member.hasArmor($dataArmors[listId])
      );
    }
  }
  ...
  function elementValidity(data, battler) {
    return getValList(data.IDList).some((id) =>
      conditionsNum(data, battler.elementRate(id) * 100)
    );
  }

  function stateValidity(data, battler) {
    return getValList(data.IDList).some((id) =>
      conditionsNum(data, battler.stateRate(id) * 100)
    );
  }
  ...
  Game_BattlerBase.prototype.conditionsParam = function (data, paramId) {
    ...
    return (
      paramVal >= (paramMaxVal * data.DwLimit) / 100 &&
      (data.UpLimit > 0 ? paramVal <= (paramMaxVal * data.UpLimit) / 100 : true)
    );
  };
