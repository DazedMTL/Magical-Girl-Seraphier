...
(function () {
  ...
  AudioManager.getVideoPictureVolume = function () {
    ...
    return (
      Video._volume *
      (this.hasOwnProperty(property) ? this[property] / 100 : 1.0)
    );
  };
  ...
  Bitmap_Video.prototype._createVideo = function (url) {
    ...
    this._video.addEventListener(
      "error",
      (this._errorListener = this._onError.bind(this))
    );
    this._video.load();
  ...
  Bitmap_Video.prototype._createVideoBaseTexture = function () {
    const scaleMode = this.smooth
      ? PIXI.SCALE_MODES.LINEAR
      : PIXI.SCALE_MODES.NEAREST;
    this._baseTexture = PIXI.Texture.from(this._video, {
      scaleMode: scaleMode,
    });
  };
