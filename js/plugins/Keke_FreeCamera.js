...
(() => {
  ...
  const keke_controlCharCamera = parameters["カメラ制御文字"]
    ? parameters["カメラ制御文字"].toUpperCase()
    : "";
  ...
  PluginManager.registerCommand(pluginName, "カメラ操作", (args) => {
    ...
    // 注視対象を取得
    let chara = [
      ...getCharasByName(args["注視キャラ-名前"], self),
      ...getCharasById(args["注視キャラ-ID"], self),
    ][0];
  ...
  PluginManager.registerCommand(pluginName, "ズーム除外", (args) => {
    if (eval(args["クリア"])) {
      getAllCharacter.forEach((chara) =>
        chara ? (chara._zoomNoKe = null) : 0
      );
    }
    const self = getPlcmEvent();
    const charas = [
      ...getCharasByName(args["対象キャラ-名前"], self),
      ...getCharasById(args["対象キャラ-ID"], self),
    ];
    charas.forEach((chara) => {
      if (!chara) {
        return;
      }
      chara._zoomNoKe = args["除外時拡大率"]
        ? calcMulti(chara._zoomNoKe || 1, args["除外時拡大率"])[0].val
        : chara._zoomNoKe || 1;
    });
  });
  ...
  Sprite_Character.prototype.initialize = function (character) {
    ...
    if (
      character._eventId &&
      !character._pcStartedKeFrcm &&
      character._pageIndex >= 0
    ) {
      charaSpritePlcm = this;
      runPluginCmdStarting(
        character.list(),
        [/ズーム除外/, /カメラ操作/],
        "開始時に適用"
      );
  ...
  //- カメラの初期化
  function initCamera() {
    if (keke_initCameraInLocate) {
      $gameMap._zoomInitedKe = false;
    }
    // 注視の初期化
    initFocus();
    if ($gameMap._zoomInitedKe) {
      return;
    }
    // カメラパラムのセット
  ...
  //- ズームの呼び出し(公開)
  Game_Temp.prototype.callZoomKe = function (
    chara,
    duration,
    scale,
    offsetX,
  ...
  function finishZoom() {
    ...
    p.xDuration = 0;
    p.yDuration = 0;
    gs._zoomScale = p.scaleEasing.match(/tn|rd|bk/i)
      ? p.scaleStart
      : p.scaleTarget;
  ...
  Game_Screen.prototype.updateZoom = function () {
    ...
    // 通常の更新
    if (!did) {
      _Game_Screen_updateZoom.apply(this);
    }
    // ズーム変更の更新
    updateZoomChange();
  };
  ...
  function updateZoomParam(p) {
    const saveNo =
      $gameScreen._noZoomSaveKe ||
      SceneManager._scene.constructor.name != "Scene_Map";
    if (!saveNo) {
      p.scale = $gameScreen.zoomScale();
    }
  }
  ...
  function updateScaleEasing(gs, p) {
    if (gs._zoomDuration <= 0 || !p.scaleTimeMax) {
      return;
    }
    gs._zoomDuration--;
    p.scaleDuration--;
    gs._zoomScale = applyEasing(
      gs._zoomScale,
      p.scaleStart,
      gs._zoomScaleTarget,
      p.scaleDuration,
  ...
  function transfarFocus(p, offsetX, offsetY) {
    ...
    p.transX =
      newX +
      (Math.sin(Math.PI * (p.duration / p.timeMax + 1.5)) * volX + volX) / 2;
    const newY = p.chara._realY - cy;
    const preY = p.preChara._realY - cy;
    const volY = preY - newY;
    p.transY =
      newY +
      (Math.sin(Math.PI * (p.duration / p.timeMax + 1.5)) * volY + volY) / 2;
    const gm = $gameMap;
  ...
  //- カメラずらしの適用(コア追加)
  const _Spriteset_Base_updatePosition =
  ...
  function scaleMap(p) {
    ...
    if (!spriteset || !spriteset._tilemap.parent) {
      return;
    }
    const scale = $gameScreen.zoomScale();
    const x = xOffsetMax(p);
    const y = yOffsetMax(p);
    spriteset._tilemap.width =
      Math.ceil((Graphics.width / scale) * 2) + spriteset._tilemap._margin * 2;
    spriteset._tilemap.height =
      Math.ceil((Graphics.height / scale) * 2) + spriteset._tilemap._margin * 2;
  ...
  function scaleParallax(p) {
    ...
    spriteset._parallax.move(
      0,
      0,
      (Graphics.width / scale) * 2,
      (Graphics.height / scale) * 2
    );
  }
  ...
  Weather.prototype._rebornSprite = function (sprite) {
    ...
    sprite.ax =
      Math.randomInt(Graphics.width / zoomScale + 100) - 100 + this.origin.x;
    sprite.ay =
      Math.randomInt(Graphics.height / zoomScale + 200) - 200 + this.origin.y;
  ...
  function loadZoom() {
    ...
    if (p.chara) {
      p.chara = searchSameChara(p.chara);
    }
    if (p.preChara) {
      p.preChara = searchSameChara(p.preChara);
    }
  }
  ...
  Window_Base.prototype.convertEscapeCharacters = function (text) {
    ...
    if (dels.length) {
      dels.forEach((del) => (text = text.replace(del, "")));
    }
    return text;
  };
  ...
  function calcMulti(cur, tage, ori, cmds = []) {
    ...
    tageLine.forEach((tages, i) => {
      const match = tages.match(
        /^(r*m*s*\/|)([\+\*\/\%←→↑↓]*)(\-*\d+\.*\d*)~*(\-*\d*\.*\d*)([\+\*\/\%←→↑↓]*)(tn|cg|fk|cf|rd|bk|ei|eo|er|e|)(\(?\-*\d*\.*\d*\)?)(&*b*j*d*c*t*o*)(\_*\-?\d*\.*\d*)$/i
      );
      if (!match) {
        sols.push({
          val: tages,
  ...
  function getCharasByName(names, self) {
    ...
    for (const name of nameList) {
      ...
      // フォロワーを取得
      match = name.match(
        /^(フォロワー|フォロアー|隊列|隊列キャラ|follower)(\d*)$/
      );
      if (match) {
        const id = match[2] ? Number(match[2]) - 1 : 0;
        charas = id
          ? [...charas, $gamePlayer._followers._data[id]]
          : [...charas, ...$gamePlayer._followers._data];
      }
      // パーティを取得
      ...
      if (match) {
        const id = match[2] ? Number(match[2]) - 1 : 0;
        charas = id
          ? [...charas, $gameMap._vehicles[id]]
          : [...charas, ...$gameMap._vehicles];
      }
      // 全て取得
  ...
  function getCharasById(ids, self) {
    ...
    for (const id of idList) {
      // イベントを取得
      if (id > 0) {
        charas.push($gameMap.event(id));
      }
      // セルフを取得
      if (id == 0) {
        self = self || getPlcmEvent() || charaSpritePlcm._character;
        if (self && !idList.includes(self._eventId)) {
          charas.push(self);
        }
      }
      // プレイヤーを取得
      ...
      if (id <= -10 && id >= -99) {
        charas =
          id == -10
            ? [...charas, ...$gamePlayer._followers._data]
            : [...charas, $gamePlayer._followers._data[Math.abs(id) - 11]];
      }
      // 乗り物を取得
      if (id <= -100) {
        charas =
          id == -100
            ? [...charas, ...$gameMap._vehicles]
            : [...charas, $gameMap._vehicles[Math.abs(id) - 101]];
      }
      // 全て取得
      if (id == -2) {
        charas = [
  ...
  function isSameChara(a, b) {
    ...
    if (a._memberIndex && a._memberIndex == b._memberIndex) {
      return true;
    }
    if (a._type && a._type == b._type) {
      return true;
    }
    return false;
  }
  ...
  function isCanPluginCmd(cmd, regs = [], condition = "") {
    ...
    const regsOk = params[1]
      ? regs.filter((reg) => params[1].match(reg)).length
      : false;
    if (regsOk && condiOk) {
      result = true;
    }
  ...
  Game_InterpreterStartingKeAcms.prototype = Object.create(
    Game_Interpreter.prototype
  );
  Game_InterpreterStartingKeAcms.prototype.constructor =
    Game_InterpreterStartingKeAcms;

  //- セットアップ
  Game_InterpreterStartingKeAcms.prototype.setup = function (
    list,
    regs,
    condition
  ) {
  ...
  //- 開始時コモンイベントのセットアップ
  Game_InterpreterStartingKeAcms.prototype.setupChild = function (
    list,
    eventId
  ) {
    if (!this._childInterpreter) {
      this._childInterpreter = new Game_InterpreterStartingKeAcms(
        this._depth + 1
      );
    }
    this._childInterpreter.setup(list, this._regsKe, this._conditionKe);
    while (1) {
      if (!this._childInterpreter.executeCommand()) {
        break;
      }
    }
  };
