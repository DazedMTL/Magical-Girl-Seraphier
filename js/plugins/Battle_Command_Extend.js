...
Battle_Command_Extend.Parameters = PluginManager.parameters(
  "Battle_Command_Extend"
);
// 初期設定
Battle_Command_Extend.Command =
  Battle_Command_Extend.Parameters["Command"] ||
  "attack skill guard item escape";
Battle_Command_Extend.Flag[0] =
  Battle_Command_Extend.Parameters["PT_Flag"] || "on";
Battle_Command_Extend.Flag[1] =
  Battle_Command_Extend.Parameters["Attack_Flag"] || "off";
Battle_Command_Extend.Flag[2] =
  Battle_Command_Extend.Parameters["Skill_Flag"] || "off";
Battle_Command_Extend.Flag[3] =
  Battle_Command_Extend.Parameters["Guard_Flag"] || "off";
Battle_Command_Extend.Flag[4] =
  Battle_Command_Extend.Parameters["Item_Flag"] || "off";
Battle_Command_Extend.Flag[5] =
  Battle_Command_Extend.Parameters["Command_Flag"] || "on";
Battle_Command_Extend.Flag[6] =
  Battle_Command_Extend.Parameters["Auto_Flag"] || "off";
Battle_Command_Extend.Flag[7] =
  Battle_Command_Extend.Parameters["Repeat_Flag"] || "off";
Battle_Command_Extend.Flag[8] =
  Battle_Command_Extend.Parameters["Auto"] || "オート";
Battle_Command_Extend.Flag[9] =
  Battle_Command_Extend.Parameters["Repeat"] || "リピート";
Battle_Command_Extend.Flag[10] =
  Battle_Command_Extend.Parameters["Escape_Flag"] || "off";
...
(function () {
  ...
  // 「リピート」の一時保存
  var _Game_Temp_initialize_Battle_Command_Extend =
  ...
  // 設定保存用
  var _Game_System_initialize_Battle_Command_Extend =
  ...
  // 戦闘開始
  var BattleManager_startBattle_Battle_Command_Extend =
  ...
  BattleManager.repeatActor = function () {
    for (var i = 0; i < $gameParty.size(); i++) {
      ...
      if (actor.canInput()) {
        ...
        if ($gameTemp._repeat_action[i] === undefined) {
          // 一時保存がなければ
          $gameTemp._repeat_action[i] = [];
          for (var n = 0; n < actor.numActions(); n++) {
            // 行動回数
  ...
  // 自動戦闘用の行動候補リストを作成
  var _Game_Actor_makeActionList_Battle_Command_Extend =
    Game_Actor.prototype.makeActionList;
  Game_Actor.prototype.makeActionList = function () {
    if ($gameSystem._battle_command_flag[2] === "on") {
      // スキルが非表示ならスキルを使用しない
      var list = [];
  ...
  var _Sprite_Actor_updateTargetPosition_Battle_Command_Extend =
  ...
  Sprite_Actor.prototype.updateTargetPosition = function () {
    if ($gameSystem._battle_command_flag[0] === "on") {
      ...
      return _Sprite_Actor_updateTargetPosition_Battle_Command_Extend.call(
        this
      );
    }
  };
  // 表示行数の取得
  var _Window_ActorCommand_numVisibleRows_Battle_Command_Extend =
  ...
  Window_ActorCommand.prototype.numVisibleRows = function () {
    if (Battle_Command_Extend.Flag[5] === "on") {
      ...
      return _Window_ActorCommand_numVisibleRows_Battle_Command_Extend.call(
        this
      );
    }
  };
  // コマンドリストのリフレッシュ
  var _Window_ActorCommand_refresh_Battle_Command_Extend =
    Window_ActorCommand.prototype.refresh;
  Window_ActorCommand.prototype.refresh = function () {
    if (Battle_Command_Extend.Flag[5] === "on") {
      var wh = this.fittingHeight(this.numVisibleRows());
      var wy = Math.min(
        Graphics.boxHeight - wh,
  ...
  Window_ActorCommand.prototype.delCommandList = function (flag) {
    if (flag === true || $gameSystem._battle_command_flag[1] === "on") {
      // 攻撃
    ...
    if (flag === true || $gameSystem._battle_command_flag[2] === "on") {
      // スキル
      this._list = this._list.filter(
        function (obj) {
    ...
    if (flag === true || $gameSystem._battle_command_flag[3] === "on") {
      // 防御
      this._list = this._list.filter(
        function (obj) {
    ...
    if (flag === true || $gameSystem._battle_command_flag[4] === "on") {
      // アイテム
      this._list = this._list.filter(
        function (obj) {
    ...
    if (flag === true || $gameSystem._battle_command_flag[10] === "on") {
      // 逃げる
      this._list = this._list.filter(
        function (obj) {
  ...
  // コマンドリストの作成
  var _Window_ActorCommand_makeCommandList_Battle_Command_Extend =
    ...
    if (this._actor) {
      ...
      for (var n = 0; n < symbol.length; n++) {
        switch (symbol[n]) {
          ...
          case "escape": // 逃げる
            if (
              $gameSystem._battle_command_flag[0] === "on" &&
              $gameSystem._battle_command_flag[10] === "off"
            ) {
              this.addCommand(
                TextManager.escape,
                "escape",
          ...
          case "auto": // オート
            if (
              $gameSystem._battle_command_flag[0] === "on" &&
              $gameSystem._battle_command_flag[6] === "off"
            ) {
          ...
          case "repeat": // リピート
            if (
              $gameSystem._battle_command_flag[0] === "on" &&
              $gameSystem._battle_command_flag[7] === "off"
            ) {
  ...
  // コマンドリストの作成
  var _Window_PartyCommand_makeCommandList_Battle_Command_Extend =
  ...
  Window_PartyCommand.prototype.makeCommandList = function () {
    ...
    if (
      $gameSystem._battle_command_flag[0] === "off" &&
      $gameSystem._battle_command_flag[6] === "off"
    ) {
      this.addCommand(Battle_Command_Extend.Flag[8], "auto");
    }
  ...
  // パーティコマンド選択の開始
  var _Scene_Battle_changeInputWindow_Battle_Command_Extend =
  ...
  // パーティーコマンドウィンドウの作成
  var _Scene_Battle_createPartyCommandWindow_Battle_Command_Extend =
  ...
  // アクターコマンドウィンドウの作成
  var _Scene_Battle_createActorCommandWindow_Battle_Command_Extend =
