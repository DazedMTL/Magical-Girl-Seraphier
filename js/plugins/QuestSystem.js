...
const QuestSystemAlias = (() => {
  ...
  class RewardData {
    static fromParam(rewardParam) {
      if (rewardParam.Type === "gold") {
        ...
        return new RewardData("item", {
          item: itemInfo,
          count: rewardParam.ItemCount,
        });
      } else if (rewardParam.Type === "any") {
        return new RewardData("any", {
          text: rewardParam.Text,
          iconIndex: rewardParam.IconIndex,
        });
      }
    }
  ...
  class TextDrawer {
    ...
    drawIconTextByMode(text, iconIndex, x, y, width, mode) {
      const iconY =
        y + (this._window.lineHeight() - ImageManager.iconHeight) / 2;
  ...
  class RewardWindowDrawer {
    ...
    drawRewardToWindow(x, y, width) {
      if (this._reward.type === "gold") {
        ...
        this._window.drawItemName(
          this._reward.params.item.itemData(),
          x,
          y,
          width
        );
        ...
        this._textDrawer.drawIconText(
          this._reward.params.text,
          this._reward.params.iconIndex,
          x,
          y,
          width
        );
  ...
  class QuestData {
    ...
    state() {
      const data = STATE_LIST.find(
        (data) => data.value === $gameVariables.value(this._variableId)
      );
  ...
  const params = PluginParamsParser.parse(
    PluginManager.parameters(QuestSystemPluginName),
    typeDefine
  );
  ...
  const STATE_LIST = [
    { state: "none", value: 0, text: "" },
    {
      state: "notOrdered",
      value: 1,
      text: Text.NotOrderedStateText,
      color: TextColor.NotOrderedStateColor,
    },
    {
      state: "ordering",
      value: 2,
      text: Text.OrderingStateText,
      color: TextColor.OrderingStateColor,
    },
    {
      state: "reportable",
      value: 3,
      text: Text.ReportableStateText,
      color: TextColor.ReportableStateColor,
    },
    {
      state: "reported",
      value: 4,
      text: Text.ReportedStateText,
      color: TextColor.ReportedStateColor,
    },
    {
      state: "failed",
      value: 5,
      text: Text.FailedStateText,
      color: TextColor.FailedStateColor,
    },
    {
      state: "expired",
      value: 6,
      text: Text.ExpiredStateText,
      color: TextColor.ExpiredStateColor,
    },
  ...
  const COMMAND_TABLE = {
    ...
    orderingQuest: {
      state: ["ordering", "reportable"],
      text: Text.OrderingQuestCommandText,
    },
  ...
  class Scene_QuestSystem extends superScene_Message {
    ...
    createBackground() {
      ...
      if (this._backgroundImage.FileName1) {
        const bitmap1 = ImageManager.loadBitmap(
          "img/",
          this._backgroundImage.FileName1
        );
        this._backgroundSprite.bitmap = bitmap1;
        if (this._backgroundImage.FileName2) {
          const bitmap2 = ImageManager.loadBitmap(
            "img/",
            this._backgroundImage.FileName2
          );
    ...
    createQuestCommandWindow() {
      this._questCommandWindow = new Window_QuestCommand(
        this.questCommandWindowRect(),
        this._commandList
      );
      this._questCommandWindow.setHandler(
        "ok",
        this.onQuestCommandOk.bind(this)
      );
      this._questCommandWindow.setHandler(
        "cancel",
        this.onQuestCommandCancel.bind(this)
      );
      this._questCommandWindow.setHandler(
        "select",
        this.onQuestCommandSelect.bind(this)
      );
    ...
    createQuestListWindow() {
      ...
      this._questListWindow.setHandler(
        "cancel",
        this.onQuestListCancel.bind(this)
      );
      this._questListWindow.setHandler(
        "select",
        this.onQuestListSelect.bind(this)
      );
    ...
    createQuestDetailWindow() {
      this._questDetailWindow = new Window_QuestDetail(
        this.questDetailWindowRect()
      );
      this.addWindow(this._questDetailWindow);
    }

    createQuestOrderWindow() {
      this._questOrderWindow = new Window_QuestOrder(
        this.questOrderWindowRect()
      );
      this._questOrderWindow.setHandler("yes", this.onQuestOrderOk.bind(this));
      this._questOrderWindow.setHandler(
        "no",
        this.onQuestOrderCancel.bind(this)
      );
      this._questOrderWindow.setHandler(
        "cancel",
        this.onQuestOrderCancel.bind(this)
    ...
    createQuestOrderFailedWindow() {
      this._questOrderFailedWindow = new Window_QuestOrderFailed(
        this.questOrderFailedWindowRect()
      );
      this._questOrderFailedWindow.setHandler(
        "ok",
        this.onQuestOrderFailedOk.bind(this)
      );
    ...
    createQuestReportWindow() {
      this._questReportWindow = new Window_QuestReport(
        this.questReportWindowRect()
      );
      this._questReportWindow.setHandler(
        "yes",
        this.onQuestReportOk.bind(this)
      );
      this._questReportWindow.setHandler(
        "no",
        this.onQuestReportCancel.bind(this)
      );
      this._questReportWindow.setHandler(
        "cancel",
        this.onQuestReportCancel.bind(this)
    ...
    createQuestGetRewardWindow() {
      this._questGetRewardWindow = new Window_QuestGetReward(
        this.questGetRewardWindowRect()
      );
      this._questGetRewardWindow.setHandler(
        "ok",
        this.onQuestGetRewardOk.bind(this)
      );
      this.addWindow(this._questGetRewardWindow);
    }

    createQuestCancelWindow() {
      this._questCancelWindow = new Window_QuestCancel(
        this.questCancelWindowRect()
      );
      this._questCancelWindow.setHandler(
        "yes",
        this.onQuestCancelOk.bind(this)
      );
      this._questCancelWindow.setHandler(
        "no",
        this.onQuestCancelCancel.bind(this)
      );
      this._questCancelWindow.setHandler(
        "cancel",
        this.onQuestCancelCancel.bind(this)
    ...
    questListWindowRect() {
      ...
      const w = WindowSize.CommandWindowWidth;
      const bottom = this.isBottomButtonMode()
    ...
    onQuestListOk() {
      switch (this._questCommandWindow.currentSymbol()) {
        case "questOrder":
          if (
            MaxOrderingQuests === 0 ||
            this.numOrderingQuests() < MaxOrderingQuests
          ) {
    ...
    numOrderingQuests() {
      ...
      const orderingQuests = $dataQuests.filter((quest) =>
        orderingQuestCommand.state.includes(quest.state())
      );
  ...
  class Window_QuestCommand extends Window_Command_MZMV {
    ...
    filterQuestList() {
      ...
      return $dataQuests.filter((quest) =>
        commandData.state.includes(quest.state())
      );
    }
  }
  ...
  class Window_QuestList extends Window_Command_MZMV {
    ...
    makeCommandList() {
      for (let i = 0; i < this._questList.length; i++) {
        ...
        const title =
          questData.state() === "hidden"
            ? Text.HiddenTitleText
            : questData.title;
  ...
  class Window_QuestDetail extends Window_Selectable_MZMV {
    ...
    drawTitle(startLine) {
      ...
      if (QuestTitleWrap) {
        if (this._questData.iconIndex === 0) {
          titleRows = this.drawTextExWrap(
            this._questData.title,
            this.padding,
            this.startY(startLine),
            titleWidth
          );
        } else {
          titleRows = this.drawIconTextWrap(
            this._questData.title,
            this._questData.iconIndex,
            this.padding,
            this.startY(startLine),
            titleWidth
          );
      ...
      this.drawText(
        this._questData.stateText(),
        this.padding + titleWidth,
        this.startY(startLine),
        stateWidth,
        "right"
      );
    ...
    drawReward(reward, y) {
      new RewardWindowDrawer(this, reward).drawRewardToWindow(
        this.messageX(),
        y,
        this.messageWindth()
      );
  ...
  PluginManager.registerCommand(
    ...
    (args) => {
      ...
      const commands =
        params.QuestCommands.length === 0 ? null : params.QuestCommands;
  ...
  PluginManager.registerCommand(QuestSystemPluginName, "GetRewards", (args) => {
    ...
    const questData = $dataQuests.find(
      (data) => data.variableId === params.VariableId
    );
    if (!questData) return;
    questData.getRewards();
  });
