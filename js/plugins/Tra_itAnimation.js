...
(function () {
  ...
  Game_BattlerBase.prototype.findTraitAnimation = function () {
    ...
    this.traitObjects().forEach(function (obj) {
      const meta = PluginManagerEx.findMetaValue(obj, [
        "TraitAnimation",
        "特徴アニメ",
      ]);
      if (meta) {
  ...
  Game_Temp.prototype.requestTraitAnimation = function (
    targets,
    animationId,
    mirror
  ) {
  ...
  const _Spriteset_Base_updateAnimations =
  ...
  const _Spriteset_Base_createAnimation =
  ...
  const _Spriteset_Base_createAnimationSprite =
