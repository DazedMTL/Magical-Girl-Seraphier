...
(() => {
  ...
  if (Utils.RPGMAKER_NAME === "MV") {
    ...
    ShaderTilemap.prototype.updateTransform = function () {
      ...
      const startY = Math.floor((oy - this._margin) / this.tileHeight);
      this._moveDarkness(
        startX * this.tileWidth - ox,
        startY * this.tileHeight - oy
      );
  ...
  DarknessLayer.prototype._paintFrontLight = function (
    x,
    y,
    rgb,
    angle,
  ...
  DarknessLayer.prototype._paintNormalMapLight = function (
    x,
    y,
    rgb,
    r,
  ...
  Game_Map.prototype.allMapLights = function () {
    return [...Object.values(this._mapLights), ...$gamePlayer.allMapLights()];
  };
  ...
  if (!PluginManager.registerCommand) {
    ...
    PluginManager.callCommandMV = function (
      self,
      pluginName,
      commandName,
      args
...

  {
    const _registerCommand =
      PluginManager.registerCommand || PluginManager.registerCommandMV;

    _registerCommand.call(
      PluginManager,
      pluginName,
      "setCharLight",
      function (args) {
        const character = this.character(
          PluginManager.mppValue(args.character)
        );
